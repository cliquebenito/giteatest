...
var CmdWeb = cli.Command{
	Name:  "web",
	Usage: "Start SC web server",
	Description: `SC web server is the only thing you need to run,
...
func runWeb(ctx *cli.Context) error {
	...
	if os.Getppid() > 1 && len(os.Getenv("LISTEN_FDS")) > 0 {
		log.Info("Restarting SC on PID: %d from parent PID: %d", os.Getpid(), os.Getppid())
	} else {
		log.Info("Starting SC on PID: %d", os.Getpid())
	}
	...
	if needsInstall {
		...
		err := listen(c, false)
		if err != nil {
			log.Critical("Unable to open listener for installer. Is SC already running?")

			audit.CreateAndSendEvent(audit.ServiceStopEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusSuccess, audit.EmptyRequiredField, nil)
		...
		case <-graceful.GetManager().IsShutdown():
			...
			log.Info("PID: %d SC Web Finished", os.Getpid())
...
func listen(m http.Handler, handleRedirector bool) error {
	...
	_, _, finished := process.GetManager().AddTypedContext(graceful.GetManager().HammerContext(), "Web: SC Server", process.SystemProcessType, true)
	defer finished()
	log.Info("Listen: %v://%s%s", setting.Protocol, listenAddr, setting.AppSubURL)

	audit.CreateAndSendEvent(audit.ServiceStartEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusSuccess, audit.EmptyRequiredField, nil)
