...
func setup(ctx context.Context, debug bool) {
	...
	setting.Init(&setting.Options{ConsoleConf: true})
	if debug {
		setting.RunMode = "dev"
	}

	// Check if setting.RepoRootPath exists. It could be the case that it doesn't exist, this can happen when
	// `[repository]` `ROOT` is a relative path and $SC_WORK_DIR isn't passed to the SSH connection.
	if _, err := os.Stat(setting.RepoRootPath); err != nil {
		if os.IsNotExist(err) {
			_ = fail(ctx, "Incorrect configuration, no repository directory.", "Directory `[repository].ROOT` %q was not found, please check if $SC_WORK_DIR is passed to the SSH connection or make `[repository].ROOT` an absolute value.", setting.RepoRootPath)
		} else {
...
func fail(ctx context.Context, userMessage, logMsgFmt string, args ...interface{}) error {
	...
	_, _ = fmt.Fprintln(os.Stderr, "SourceControl:", userMessage)

	if logMsgFmt != "" {
		logMsg := fmt.Sprintf(logMsgFmt, args...)
		if !setting.IsProd {
			_, _ = fmt.Fprintln(os.Stderr, "SourceControl:", logMsg)
		}
		if userMessage != "" {
...
func runServ(c *cli.Context) error {
	...
	if setting.SSH.Disabled {
		println("SourceControl: SSH has been disabled")
		return nil
	}
	...
	if len(cmd) == 0 {
		...
		case asymkey_model.KeyTypeDeploy:
			println("Hi there! You've successfully authenticated with the deploy key named " + key.Name + ", but SourceControl does not provide shell access.")
		case asymkey_model.KeyTypePrincipal:
			println("Hi there! You've successfully authenticated with the principal " + key.Content + ", but SourceControl does not provide shell access.")
		default:
			println("Hi there, " + user.Name + "! You've successfully authenticated with the key named " + key.Name + ", but SourceControl does not provide shell access.")
		}
		println("If this is unexpected, please log in with password and setup SourceControl under another user.")
		return nil
