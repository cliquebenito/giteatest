...
Of note, configuring `SC_WORK_DIR` will tell Gitea where to base its working directory, as well as ease installation.
...
```sh
export SC_WORK_DIR=/var/lib/gitea/
```
...
```sh
SC_WORK_DIR=/var/lib/gitea/ /usr/local/bin/gitea web -c /etc/gitea/app.ini
```
