...
the `CustomPath` by setting either the `SC_CUSTOM` environment variable or by
...
`$SC_CUSTOM/conf/app.ini` but may be different if your build has set this differently.
...
If the `CustomPath` folder can't be found despite checking `gitea help`, check the `SC_CUSTOM`
environment variable; this can be used to override the default path to something else.
`SC_CUSTOM` might, for example, be set by an init script. You can check whether the value
...
`$SC_CUSTOM/public/` as the webroot. Symbolic links will be followed.

For example, a file `image.png` stored in `$SC_CUSTOM/public/`, can be accessed with
...
`make generate-images`. `assets/favicon.svg` is used for the favicon only. This will update below output files which you can then place in `$SC_CUSTOM/public/img` on your server:
...
must be placed in `$SC_CUSTOM/options/gitignore/C++` (see about the location of the `CustomPath`
directory at the top of this document).

Every single page of Gitea can be changed. Dynamic content is generated using [go templates](https://golang.org/pkg/html/template/),
which can be modified by placing replacements below the `$SC_CUSTOM/templates` directory.
...
Copy [`home.tmpl`](https://github.com/go-gitea/gitea/blob/main/templates/home.tmpl) for your version of Gitea from `templates` to `$SC_CUSTOM/templates`.
Edit as you wish.
Dont forget to restart your Gitea to apply the changes.

### Adding links and tabs

If all you want is to add extra links to the top navigation bar or footer, or extra tabs to the repository view, you can put them in `extra_links.tmpl` (links added to the navbar), `extra_links_footer.tmpl` (links added to the left side of footer), and `extra_tabs.tmpl` inside your `$SC_CUSTOM/templates/custom/` directory.

For instance, let's say you are in Germany and must add the famously legally-required "Impressum"/about page, listing who is responsible for the site's content:
just place it under your "$SC_CUSTOM/public/" directory (for instance `$SC_CUSTOM/public/impressum.html`) and put a link to it in either `$SC_CUSTOM/templates/custom/extra_links.tmpl` or `$GITEA_CUSTOM/templates/custom/extra_links_footer.tmpl`.
...
Apart from `extra_links.tmpl` and `extra_tabs.tmpl`, there are other useful templates you can put in your `$SC_CUSTOM/templates/custom/` directory:
...
`$SC_CUSTOM/public` folder. Then add the following to `custom/footer.tmpl`:
...
You also need to download the content of the library [Madeleine.js](https://github.com/beige90/Madeleine.js) and place it under `$SC_CUSTOM/public/` folder.
...
The `$SC_CUSTOM/templates/mail` folder allows changing the body of every mail of Gitea.
...
Override by making a copy of the file under `$SC_CUSTOM/templates/mail` using a
...
Google Analytics, Matomo (previously Piwik), and other analytics services can be added to Gitea. To add the tracking code, refer to the `Other additions to the page` section of this document, and add the JavaScript to the `$SC_CUSTOM/templates/custom/header.tmpl` file.
...
To add custom .gitignore, add a file with existing [.gitignore rules](https://git-scm.com/docs/gitignore) in it to `$SC_CUSTOM/options/gitignore`

### Labels

Starting with Gitea 1.19, you can add a file that follows the [YAML label format](https://github.com/go-gitea/gitea/blob/main/options/label/Advanced.yaml) to `$SC_CUSTOM/options/label`:
...
To add a custom license, add a file with the license text to `$SC_CUSTOM/options/license`

### Locales

Locales are managed via our [Crowdin](https://crowdin.com/project/gitea).
You can override a locale by placing an altered locale file in `$SC_CUSTOM/options/locale`.
...
To add a custom Readme, add a markdown formatted file (without an `.md` extension) to `$SC_CUSTOM/options/readme`
...
1. Add a CSS file to `$SC_CUSTOM/public/css/theme-<theme-name>.css`.
  The value of `$SC_CUSTOM` of your instance can be queried by calling `gitea help` and looking up the value of "CustomPath".
