...
func pushUpdates(optsList []*repo_module.PushUpdateOptions) error {
	...
	auditParams := map[string]string{
		"repository": optsList[0].RepoUserName,
		"owner":      optsList[0].RepoName,
	}
	...
	if err != nil {
		...
		audit.CreateAndSendEvent(audit.ChangesPushEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
		return fmt.Errorf("GetRepositoryByOwnerAndName failed: %w", err)
	}

	repoPath := repo.RepoPath()

	gitRepo, err := git.OpenRepository(ctx, repo.OwnerName, repo.Name, repoPath)
	if err != nil {
	...
	for _, opts := range optsList {
		...
		if opts.IsNewRef() && opts.IsDelRef() {
			auditParams["error"] = "Old and new revisions are both has empty SHA"
			audit.CreateAndSendEvent(audit.ChangesPushEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
			return fmt.Errorf("old and new revisions are both %s", git.EmptySHA)
		}
		if opts.IsTag() { // If is tag reference
			...
			if !opts.IsDelRef() {
				...
				var l []*git.Commit
				if opts.IsNewRef() {
					auditParams["branch_name"] = refName
						...
						if repo.DefaultBranch != setting.Repository.DefaultBranch {
							if err := gitRepo.SetDefaultBranch(repo.DefaultBranch); err != nil {
								if !git.IsErrUnsupportedVersion(err) {
									auditParams["error"] = "Error has occurred while setting default branch"
									audit.CreateAndSendEvent(audit.BranchCreateEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
									audit.CreateAndSendEvent(audit.ChangesPushEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
									return err
								}
							}
					...
					if err != nil {
						auditParams["error"] = "Error has occurred while getting num commits before current revision"
						audit.CreateAndSendEvent(audit.BranchCreateEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
						audit.CreateAndSendEvent(audit.ChangesPushEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
					...
					audit.CreateAndSendEvent(audit.BranchCreateEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusSuccess, audit.EmptyRequiredField, auditParams)
				} else {
					l, err = newCommit.CommitsBeforeUntil(opts.OldCommitID)
					if err != nil {
						auditParams["error"] = "Error has occurred while getting commits between commitID to current revision"
						audit.CreateAndSendEvent(audit.ChangesPushEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
	...
	if err := PushUpdateAddDeleteTags(ctx, repo, gitRepo, addTags, delTags); err != nil {
		auditParams["error"] = "Error has occurred while updating a number of added and delete tags"
		audit.CreateAndSendEvent(audit.ChangesPushEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
	...
	if err := repo_model.UpdateRepositoryUpdatedTime(repo.ID, time.Now()); err != nil {
		auditParams["error"] = "Error has occurred while changing repository last updated time."
		audit.CreateAndSendEvent(audit.ChangesPushEvent, optsList[0].PusherName, strconv.FormatInt(optsList[0].PusherID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
		...
		if sig != nil {
			...
			if !ok {
				if sig.Email != "" {
					...
					author, err = user_model.GetUserByName(ctx, sig.Name)
					if err != nil && !user_model.IsErrUserNotExist(err) {
						return fmt.Errorf("GetUserByName: %w", err)
					}
				}
				if author != nil {
