...
func (t *TemporaryUploadRepository) Clone(branch string) error {
	if _, _, err := git.NewCommand(t.ctx, "clone", "-s", "--bare", "-b").AddDynamicArguments(branch, t.repo.CloneLink().HTTPS, t.basePath).RunStdString(nil); err != nil {
		stderr := err.Error()
	...
	gitRepo, err := git.OpenRepository(t.ctx, t.repo.OwnerName, t.repo.Name, t.basePath)
	if err != nil {
		return err
	}
	t.gitRepo = gitRepo
...
// AddObjectToIndex adds the provided object hash to the index with the provided mode and path
func (t *TemporaryUploadRepository) RefreshObjectToIndex() error {
	if _, _, err := git.NewCommand(t.ctx, "update-index", "--refresh").RunStdString(&git.RunOpts{Dir: t.basePath}); err != nil {
		...
		return fmt.Errorf("Unable to add object to index in temporary repo %s Error: %w", t.repo.FullName(), err)
	}
	return nil
}
...
func (t *TemporaryUploadRepository) CommitTreeWithDate(parent string, author, committer *user_model.User, treeHash, message string, signoff bool, authorDate, committerDate time.Time) (string, error) {
	...
	if parent != "" {
		sign, keyID, signer, _ = asymkey_service.SignCRUDAction(t.ctx, t.repo.OwnerName, t.repo.Name, t.repo.RepoPath(), author, t.basePath, parent)
	} else {
...
func (t *TemporaryUploadRepository) DiffIndex(treeHash string) (*gitdiff.Diff, error) {
	...
	if err := git.NewCommand(t.ctx, "diff-index", "--src-prefix=\\a/", "--dst-prefix=\\b/", "--cached", "-p").AddDynamicArguments(treeHash).
		Run(&git.RunOpts{
