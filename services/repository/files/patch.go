...
func (opts *ApplyDiffPatchOptions) Validate(ctx context.Context, repo *repo_model.Repository, doer *user_model.User) error {
	...
	if opts.NewBranch != opts.OldBranch {
		...
		protectedBranch, err := git_model.GetMergeMatchProtectedBranchRule(ctx, repo.ID, opts.OldBranch)
		if err != nil {
			log.Error("Error has occured while get merge match protected branch with repoID - %d, branchName - %s", repo.ID, opts.OldBranch)
			return fmt.Errorf("Err: get merge protected branch: %w", err)
		}
		if protectedBranch != nil {
			protectedBranch.Repo = repo
			if !git_model.CanUserPush(ctx, *protectedBranch, doer) {
		...
		if protectedBranch != nil && protectedBranch.RequireSignedCommits {
			_, _, _, err := asymkey_service.SignCRUDAction(ctx, repo.OwnerName, repo.Name, repo.RepoPath(), doer, repo.RepoPath(), opts.OldBranch)
			if err != nil {
