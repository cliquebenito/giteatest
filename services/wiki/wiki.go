...
func updateWikiPage(ctx context.Context, doer *user_model.User, repo *repo_model.Repository, oldWikiName, newWikiName WebPath, content, message string, isNew bool) (err error) {
	...
	hasMasterBranch := git.IsBranchExist(ctx, repo.OwnerName, repo.Name, repo.WikiPath(), DefaultBranch)
	...
	gitRepo, err := git.OpenRepository(ctx, repo.OwnerName, repo.Name, basePath)
	if err != nil {
	...
	sign, signingKey, signer, _ := asymkey_service.SignWikiCommit(ctx, repo.OwnerName, repo.Name, repo.WikiPath(), doer)
	if sign {
