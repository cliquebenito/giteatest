...
func TestRepository_AddWikiPage(t *testing.T) {
	...
	for _, userTitle := range []string{
		...
		t.Run("test wiki exist: "+userTitle, func(t *testing.T) {
			...
			gitRepo, err := git.OpenRepository(git.DefaultContext, repo.OwnerName, repo.Name, repo.WikiPath())
			assert.NoError(t, err)
			defer gitRepo.Close()
	...
	t.Run("check wiki already exist", func(t *testing.T) {
		...
		err := AddWikiPage(git.DefaultContext, doer, repo, "HomeOrg", wikiContent, commitMsg)
		assert.Error(t, err)
...
func TestRepository_EditWikiPage(t *testing.T) {
	...
	for _, newWikiName := range []string{
		"HomeOrg", // same name as before
		"New home",
		"New/name/with/slashes",
	} {
		webPath := UserTitleToWebPath("", newWikiName)
		unittest.PrepareTestEnv(t)
		assert.NoError(t, EditWikiPage(git.DefaultContext, doer, repo, "HomeOrg", webPath, newWikiContent, commitMsg))
		...
		if newWikiName != "HomeOrg" {
			_, err := masterTree.GetTreeEntryByPath("HomeOrg.md")
			assert.Error(t, err)
		}
		gitRepo.Close()
	}
...
func TestRepository_DeleteWikiPage(t *testing.T) {
	...
	repo := unittest.AssertExistsAndLoadBean(t, &repo_model.Repository{ID: 1})
	doer := unittest.AssertExistsAndLoadBean(t, &user_model.User{ID: 2})
	assert.NoError(t, DeleteWikiPage(git.DefaultContext, doer, repo, "HomeOrg"))
	...
	masterTree, err := gitRepo.GetTree(DefaultBranch)
	assert.NoError(t, err)
	gitPath := WebPathToGitPath("HomeOrg")
...
func TestPrepareWikiFileName_FirstPage(t *testing.T) {
	...
	gitRepo, err := git.OpenRepository(git.DefaultContext, "owner", "repo", tmpDir)
	defer gitRepo.Close()
	assert.NoError(t, err)

	existence, newWikiPath, err := prepareGitPath(gitRepo, "HomeOrg")
	assert.False(t, existence)
	assert.NoError(t, err)
	assert.EqualValues(t, "HomeOrg.md", newWikiPath)
}
