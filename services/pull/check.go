...
func CheckPullMergable(stdCtx context.Context, doer *user_model.User, perm *access_model.Permission, pr *issues_model.PullRequest, mergeCheckType MergeCheckType, adminSkipProtectionCheck bool) error {
	...
	return db.WithTx(stdCtx, func(ctx context.Context) error {
		...
		if allowedMerge, err := reviewSettings.IsUserAllowedToMerge(ctx, pr, doer); err != nil {
		...
		if err := reviewSettings.CheckReviewSettingsProtections(ctx, pr, false); err != nil {
			...
			if adminSkipProtectionCheck {
				if isRepoAdmin, errCheckAdmin := access_model.IsUserRepoAdmin(ctx, pr.BaseRepo, doer); errCheckAdmin != nil {
					...
					repoUnit, unitErr := pr.BaseRepo.GetUnit(ctx, unit.TypePullRequests)
					if unitErr != nil {
						return unitErr
					}
		...
		allowedCodeOwners, err := IsCodeOwnersAllowedToMerge(ctx, pr, doer)
		if err != nil {
			log.Error("Error whilst checking if %-v is allowed to merge %-v: %v", doer, pr, err)
			return fmt.Errorf("IsCodeOwnersAllowedToMerge: %w", err)
		}
		if !allowedCodeOwners {
			log.Warn("User %s is not allowed to merge PR %d because of code owners", doer.Name, pr.ID)
			return ErrUserNotAllowedToMerge
		}
...
func isSignedIfRequired(ctx context.Context, pr *issues_model.PullRequest, doer *user_model.User) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("Err: get merge protected branch: %w", err)
	}
...
func manuallyMerged(ctx context.Context, pr *issues_model.PullRequest) bool {
	...
	if merger == nil {
		merger, _ = user_model.GetUserByName(ctx, commit.Author.Name)
		if pr.BaseRepo.Owner == nil && merger == nil {
