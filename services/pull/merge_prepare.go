...
func rebaseTrackingOnToBase(ctx *mergeContext, mergeStyle repo_model.MergeStyle) error {
	...
	defer cancel()
	rebaseClient, err := ctx.gitRepo.OperationClient.UserRebaseConfirmable(ctxWithCancel)
	if err != nil {
		return err
	}
	...
	header := &gitalypb.UserRebaseConfirmableRequest{
		UserRebaseConfirmableRequestPayload: &gitalypb.UserRebaseConfirmableRequest_Header_{
			Header: &gitalypb.UserRebaseConfirmableRequest_Header{
				...
				BranchSha:        ctx.pr.MergeBase,
				RemoteRepository: ctx.gitRepo.GitalyRepo,
				RemoteBranch:     []byte(ctx.pr.HeadBranch),
			},
		},
	}
	...
	response, err := rebaseClient.Recv()
	if err != nil {
		return err
	}
	rebaseSha := response.GetRebaseSha()
	...
	if response.GetRebaseApplied() {
		ctx.pr.MergedCommitID = rebaseSha
	}
