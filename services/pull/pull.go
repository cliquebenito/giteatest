...
func NewPullRequest(ctx context.Context, repo *repo_model.Repository, pull *issues_model.Issue, labelIDs []int64, uuids []string, pr *issues_model.PullRequest, assigneeIDs []int64) error {
	auditParams := map[string]string{
		...
		"affected_user_id": strconv.FormatInt(pull.PosterID, 10),
		"affected_user":    pull.Poster.Name,
	}

	if pr == nil {
		log.Error("Error has occurred while creating pull request. Error: pr is nil")
		return fmt.Errorf("NewPullRequest: pr is nil")
	}

	if err := TestPatch(pr); err != nil {
		log.Error("Error has occurred while creating pull request. Error: %v", err)
		auditParams["error"] = "Error has occurred while creating pull request"
		audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
		return err
	}

	divergence, err := GetDiverging(ctx, pr)
	if err != nil {
		log.Error("Error has occurred while getting diverging commits for Pull Request. Error: %v", err)
		auditParams["error"] = "Error has occurred while getting diverging commits for Pull Request"
		audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
		return err
	}
	...
	for _, assigneeID := range assigneeIDs {
		if err := issue_service.AddAssigneeIfNotAssigned(ctx, pull, pull.Poster, assigneeID); err != nil {
			log.Error("Error has occurred while adding assignee to Pull Request. Error: %v", err)
			auditParams["error"] = "Error has occurred while adding assignee to Pull Request"
			audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
			return err
		}
	}
	...
	if err != nil {
		log.Error("Error has occurred while finding and updating issue mentions for Pull Request. Error: %v", err)
		auditParams["error"] = "Error has occurred while finding and updating issue mentions for Pull Request"
		audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
		return err
	}
	...
	baseGitRepo, err := git.OpenRepository(prCtx, pr.BaseRepo.OwnerName, pr.BaseRepo.Name, pr.BaseRepo.RepoPath())
	if err != nil {
		log.Error("Error has occurred while opening repository for Pull Request. Error: %v", err)
		auditParams["error"] = "Error has occurred while opening repository for Pull Request"
		audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
		return err
	}
	...
	compareInfo, err := baseGitRepo.GetCompareInfo(pr.BaseRepo.RepoPath(),
		git.BranchPrefix+pr.BaseBranch, pr.GetGitHeadBranchRefName(), false, false)
	if err != nil {
		log.Error("Error has occurred while generating compare info between branches. Error: %v", err)
		auditParams["error"] = "Error has occurred while generating compare info between branches"
		audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
		return err
	}
	...
	if len(compareInfo.Commits) > 0 {
		...
		if err != nil {
			log.Error("Error has occurred while marshalling data. Error: %v", err)
			auditParams["error"] = "Error has occurred while marshalling data"
			audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
			return err
		}
		...
		_, err = issue_service.CreateComment(ctx, ops)
		if err != nil {
			log.Error("Error has occurred while creating comment. Error: %v", err)
			auditParams["error"] = "Error has occurred while creating comment"
			audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
			return fmt.Errorf("create comment: %w", err)
		}
		if !pr.IsWorkInProgress() {
			if err := issues_model.PullRequestCodeOwnersReview(ctx, pull, pr); err != nil {
				log.Error("Error has occurred while assignment code owners. Error: %v", err)
				auditParams["error"] = "Error has occurred while assignment code owners"
				audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
				return fmt.Errorf("assignment code owners: %w", err)
			}
		}
	}
	...
	reviewSettings := NewReviewSettings(defaultReviewersdb, reviewSettingsdb)
	reviewersId, err := reviewSettings.GetReviewersForPullRequest(ctx, repo.ID, pr)
	if err != nil {
		log.Error("Error has occurred while getting reviewers for PR. Error: %v", err)
		auditParams["error"] = "Error has occurred while getting reviewers for PR"
		audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
		return fmt.Errorf("get reviewers for pr: %w", err)
	}

	for _, id := range reviewersId {
		...
		_, err = issue_service.ReviewRequest(ctx, pull, pull.Poster, reviewer, true)
		if err != nil {
			log.Error("Error has occurred while adding reviewer. Error: %v", err)
			auditParams["error"] = "Error has occurred while adding reviewer"
			audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusFailure, audit.EmptyRequiredField, auditParams)
			return fmt.Errorf("add reviewer: %w", err)
		}
	}

	audit.CreateAndSendEvent(audit.PRCreateEvent, pull.Poster.Name, strconv.FormatInt(pull.PosterID, 10), audit.StatusSuccess, audit.EmptyRequiredField, auditParams)
	return nil
}
...
func ChangeTargetBranch(ctx context.Context, pr *issues_model.PullRequest, doer *user_model.User, targetBranch string) (err error) {
	...
	err = pr.UpdateReferenceForRequestByChangeBranch(ctx, targetBranch)
	if err != nil {
		log.Error("Error has occurred while updating pull request reference for request by change branch: %v", err)
		return fmt.Errorf("failed to update pull request reference for request by change branch: %w", err)
	}
...
func AddTestPullRequestTask(doer *user_model.User, repoID int64, branch string, isSync bool, oldCommitID, newCommitID string) {
	...
	graceful.GetManager().RunWithShutdownContext(func(ctx context.Context) {
		...
		for _, pr := range prs {
			divergence, err := GetDiverging(ctx, pr)
			if err != nil {
				if models.IsErrBranchDoesNotExist(err) && !git.IsBranchExist(ctx, pr.HeadRepo.OwnerName, pr.HeadRepo.Name, pr.HeadRepo.RepoPath(), pr.HeadBranch) {
...
func checkIfPRContentChanged(ctx context.Context, pr *issues_model.PullRequest, oldCommitID, newCommitID string) (hasChanged bool, err error) {
	...
	tmpRepo, err := git.OpenRepository(ctx, prCtx.pr.BaseRepo.OwnerName, prCtx.pr.BaseRepo.Name, prCtx.tmpBasePath)
	if err != nil {
...
func CloseRepoBranchesPulls(ctx context.Context, doer *user_model.User, repo *repo_model.Repository) error {
	branches, _, err := git.GetBranchesByPath(ctx, repo.OwnerName, repo.Name, repo.RepoPath(), 0, 0)
	if err != nil {
		return err
	}
...
func GetSquashMergeCommitMessages(ctx context.Context, pr *issues_model.PullRequest) string {
	...
	gitRepo, closer, err := git.RepositoryFromContextOrOpen(ctx, pr.HeadRepo.OwnerName, pr.HeadRepo.Name, pr.HeadRepo.RepoPath())
	if err != nil {
	...
	for i := len(commits) - 1; i >= 0; i-- {
		...
		if uniqueAuthors.Add(authorString) && authorString != posterSig {
			...
			var commitUser *user_model.User
	...
	if limit >= 0 && setting.Repository.PullRequest.DefaultMergeMessageAllAuthors {
		...
		for {
			...
			for _, commit := range commits {
				...
				if uniqueAuthors.Add(authorString) && authorString != posterSig {
					commitUser, _ := user_model.GetUserByEmail(ctx, commit.Author.Email)
					if commitUser == nil && commit.Author.Name != "" {
						commitUser, _ = user_model.GetUserByName(ctx, commit.Committer.Name)
					}
					if commitUser == nil || commitUser.ID != pr.Issue.Poster.ID {
						authors = append(authors, authorString)
					}
...
func GetIssuesAllCommitStatus(ctx context.Context, issues issues_model.IssueList) (map[int64][]*git_model.CommitStatus, map[int64]*git_model.CommitStatus, error) {
	...
	for _, issue := range issues {
		...
		if !ok {
			gitRepo, err = git.OpenRepository(ctx, issue.Repo.OwnerName, issue.Repo.Name, issue.Repo.RepoPath())
			if err != nil {
...
func IsHeadEqualWithBranch(ctx context.Context, pr *issues_model.PullRequest, branchName string) (bool, error) {
	...
	baseGitRepo, closer, err := git.RepositoryFromContextOrOpen(ctx, pr.BaseRepo.OwnerName, pr.BaseRepo.Name, pr.BaseRepo.RepoPath())
	if err != nil {
		return false, err
	}
	defer closer.Close()
	...
	if pr.HeadRepoID == pr.BaseRepoID {
		...
		headGitRepo, closer, err = git.RepositoryFromContextOrOpen(ctx, pr.HeadRepo.OwnerName, pr.HeadRepo.Name, pr.HeadRepo.RepoPath())
		if err != nil {
			return false, err
		}
		defer closer.Close()
