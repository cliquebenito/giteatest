...
func parseHunks(curFile *DiffFile, maxLines, maxLineCharacters int, input *bufio.Reader) (lineBytes []byte, isFragment bool, err error) {
	...
	for {
		...
		case ' ':
			...
			curSection.Lines = append(curSection.Lines, diffLine)
		default:
			if strings.Contains(string(lineBytes), "Binary files") {
				continue
			}
...
func GetDiff(gitRepo *git.Repository, opts *DiffOptions, files ...string) (*Diff, error) {
	...
	if len(actualBeforeCommitID) == 0 {
		if commit.ParentCount() == 0 {
			actualBeforeCommitID = "4b825dc642cb6eb9a060e54bf8d69288fbee4904"
		} else {
	...
	diff := &Diff{Files: make([]*DiffFile, 0)}
	statsByPath := make(map[string]*gitalypb.DiffStats, 0)

	diffStats, err := gitRepo.DiffClient.DiffStats(gitRepo.Ctx, &gitalypb.DiffStatsRequest{
		Repository:    gitRepo.GitalyRepo,
		LeftCommitId:  actualBeforeCommitID,
		RightCommitId: opts.AfterCommitID,
	})
	if err != nil {
		return nil, err
	}

	statsResponse, err := diffStats.Recv()
	if err != nil {
		return nil, err
	}
	for _, stat := range statsResponse.Stats {
		statsByPath[string(stat.Path)] = stat
	}

	const maxBytes = int32(^uint32(0) >> 1)

	commitDiff, err := gitRepo.DiffClient.CommitDiff(gitRepo.Ctx, &gitalypb.CommitDiffRequest{
		Repository:    gitRepo.GitalyRepo,
		LeftCommitId:  actualBeforeCommitID,
		RightCommitId: opts.AfterCommitID,
		Paths:         byteFiles,
	...
	for {
		...
		if err != nil && err != io.EOF {
			log.Error("Error has occurred while receiving commit diff: %v", err)
			return nil, fmt.Errorf("error receive commit diff: %w", err)
		}
		if recv == nil {
			break
		}
		...
		curFile := &DiffFile{
			Name:      string(recv.ToPath),
			NameHash:  base.EncodeSha1(string(recv.ToPath)),
			OldName:   string(recv.FromPath),
			Index:     len(diff.Files) + 1,
			IsBin:     recv.Binary,
			IsRenamed: string(recv.ToPath) != string(recv.FromPath),
		}

		rawPatchData, err := git.ReadRawPatchData(commitDiff, string(recv.GetRawPatchData()), recv.GetEndOfPatch())
		if err != nil {
			log.Error("Error has occurred while reading raw patch data: %v", err)
			return nil, fmt.Errorf("failed to read raw patch data: %w", err)
		}

		_, _, err = parseHunks(curFile, opts.MaxLines, opts.MaxLineCharacters, bufio.NewReader(strings.NewReader(rawPatchData)))
		if err != nil && err != io.EOF {
			return nil, err
		}
		fileStats := statsByPath[curFile.Name]
		if fileStats == nil {
			return nil, fmt.Errorf("the changes in the file are too big")
		}

		curFile.Addition = int(fileStats.Additions)
		curFile.Deletion = int(fileStats.Deletions)
