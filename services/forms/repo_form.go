...
type CreateRepoForm struct {
	...
	DefaultBranch string `binding:"SbtGitRefName;SbtMaxSize(100)"`
	AutoInit      bool
	Gitignores    string
	IssueLabels   string
	License       string
...
// CodeOwnersForm форма для изменения количества обязательных аппруверов в настройках репозитория
type CodeOwnersForm struct {
	ApprovalStatus bool
	AmountUsers    int64
}
...
type ProtectBranchForm struct {
	...
	EnableDelete               string
	DeleterWhitelistUsers      string
	DeleterWhitelistDeployKeys bool

	EnableForcePush                string
	ForcePusherWhitelistUsers      string
	ForcePusherWhitelistDeployKeys bool
}
...
type NewReleaseForm struct {
	TagName    string `binding:"Required;SbtGitRefName;MaxSize(255)"`
...
type EditRepoFileForm struct {
	...
	NewBranchName string `binding:"SbtGitRefName;SbtMaxSize(100)"`
	LastCommit    string
	Signoff       bool
}
