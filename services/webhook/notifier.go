...
// NotifyPullRequestRebuild запуск веб-хуков репозитория/организации
func (m *webhookNotifier) NotifyPullRequestRebuild(ctx context.Context, doer *user_model.User, issue *issues_model.Issue, remoteAddress string) {
	mode, _ := access_model.AccessLevel(ctx, issue.Poster, issue.Repo)
	var err error
	if issue.IsPull {
		if err = issue.LoadPullRequest(ctx); err != nil {
