...
func IsValidHookTaskType(name string) bool {
	if name == webhook_module.SOURCECONTROL || name == webhook_module.GOGS {
		return true
	}
	_, ok := webhooks[name]
	return ok
...
func PrepareWebhook(ctx context.Context, w *webhook_model.Webhook, event webhook_module.HookEventType, p api.Payloader) error {
	...
	if pushEvent, ok := p.(*api.PushPayload); ok &&
		w.Type != webhook_module.SOURCECONTROL && w.Type != webhook_module.GOGS &&
...
func PrepareWebhooks(ctx context.Context, source EventSource, event webhook_module.HookEventType, p api.Payloader) error {
	...
	sender := getSenderFromPayloader(p, event)
	auditUserName := audit.EmptyRequiredField
	auditUserId := audit.EmptyRequiredField
	auditRemoteAddress := audit.EmptyRequiredField
	if sender != nil {
		auditUserName = sender.UserName
		auditUserId = strconv.FormatInt(sender.ID, 10)
		remoteAddress := getRemoteAddressFromPayloader(p, event)
		if remoteAddress != "" {
			auditRemoteAddress = remoteAddress
		}
	}
	...
	if source.Repository != nil {
		auditParams["repository"] = source.Repository.Name
		auditParams["repository_id"] = strconv.FormatInt(source.Repository.ID, 10)
		auditParams["owner"] = source.Repository.OwnerName
		...
		if err != nil {
			auditParams["error"] = "Error has occurred while getting repository list webhooks by options"
			audit.CreateAndSendEvent(audit.BuildRunEvent, auditUserName, auditUserId, audit.StatusFailure, auditRemoteAddress, auditParams)
	...
	if owner != nil {
		...
		if err != nil {
			auditParams["error"] = "Error has occurred while getting owner list webhooks by options"
			audit.CreateAndSendEvent(audit.BuildRunEvent, auditUserName, auditUserId, audit.StatusFailure, auditRemoteAddress, auditParams)
			return fmt.Errorf("ListWebhooksByOpts: %w", err)
		}
	...
	if err != nil {
		auditParams["error"] = "Error has occurred while getting system list webhooks by options"
		audit.CreateAndSendEvent(audit.BuildRunEvent, auditUserName, auditUserId, audit.StatusFailure, auditRemoteAddress, auditParams)
	...
	for _, w := range ws {
		if err := PrepareWebhook(ctx, w, event, p); err != nil {
			...
			auditParams["hook_type"] = w.Type
			auditParams["hook_url"] = w.URL
			auditParams["error"] = "Error has occurred while preparing webhook"
			audit.CreateAndSendEvent(audit.BuildRunEvent, auditUserName, auditUserId, audit.StatusFailure, auditRemoteAddress, auditParams)
			return err
		}
	}
	audit.CreateAndSendEvent(audit.BuildRunEvent, auditUserName, auditUserId, audit.StatusSuccess, auditRemoteAddress, auditParams)
	return nil
}
