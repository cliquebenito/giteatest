...
func getSenderFromPayloader(p api.Payloader, event webhook_module.HookEventType) *api.User {
	switch event {
	case webhook_module.HookEventCreate:
		return p.(*api.CreatePayload).Sender
	case webhook_module.HookEventDelete:
		return p.(*api.DeletePayload).Sender
	case webhook_module.HookEventFork:
		return p.(*api.ForkPayload).Sender
	case webhook_module.HookEventIssues, webhook_module.HookEventIssueAssign, webhook_module.HookEventIssueLabel, webhook_module.HookEventIssueMilestone:
		return p.(*api.IssuePayload).Sender
	case webhook_module.HookEventIssueComment, webhook_module.HookEventPullRequestComment:
		return p.(*api.IssueCommentPayload).Sender
	case webhook_module.HookEventPush:
		return p.(*api.PushPayload).Sender
	case webhook_module.HookEventPullRequest, webhook_module.HookEventPullRequestAssign, webhook_module.HookEventPullRequestLabel,
		webhook_module.HookEventPullRequestMilestone, webhook_module.HookEventPullRequestSync:
		return p.(*api.PullRequestPayload).Sender
	case webhook_module.HookEventPullRequestReviewApproved, webhook_module.HookEventPullRequestReviewRejected, webhook_module.HookEventPullRequestReviewComment:
		return p.(*api.PullRequestPayload).Sender
	case webhook_module.HookEventRepository:
		return p.(*api.RepositoryPayload).Sender
	case webhook_module.HookEventRelease:
		return p.(*api.ReleasePayload).Sender
	case webhook_module.HookEventWiki:
		return p.(*api.WikiPayload).Sender
	}
	return nil
}

func getRemoteAddressFromPayloader(p api.Payloader, event webhook_module.HookEventType) string {
	switch event {
	case webhook_module.HookEventPullRequest:
		return p.(*api.PullRequestPayload).RemoteAddress
	}
	return ""
}
