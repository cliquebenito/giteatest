...
func ListTasks() TaskTable {
	...
	for _, task := range tasks {
		...
		cronJobLock, err := system.GetCronJobLock(task.Name)
		if err == nil && cronJobLock != nil {
			if !cronJobLock.Prev.IsZero() {
				t.Prev = cronJobLock.Prev
			}
			if !cronJobLock.Next.IsZero() {
				t.Next = cronJobLock.Next
			}
			if cronJobLock.Status != "" {
				t.Status = cronJobLock.Status
			}
			if !cronJobLock.LastFailedRun.IsZero() {
				t.LastFailedRun = cronJobLock.LastFailedRun
			}
			if cronJobLock.LastFailedRunMessage != "" {
				t.LastFailedRunMessage = cronJobLock.LastFailedRunMessage
			}
		}
		tTable = append(tTable, t)
	}
