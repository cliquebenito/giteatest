...
func SendTestMail(email string) error {
	...
	return gomail.Send(Sender, NewMessage(email, "SourceControl Test Email!", "SourceControl Test Email!").ToMessage())
}
...
func generateAdditionalHeaders(ctx *mailCommentContext, reason string, recipient *user_model.User) map[string]string {
	...
	return map[string]string{
		...
		"X-Mailer":                          "SourceControl",
		"X-SourceControl-Reason":            reason,
		"X-SourceControl-Sender":            ctx.Doer.DisplayName(),
		"X-SourceControl-Recipient":         recipient.DisplayName(),
		"X-SourceControl-Recipient-Address": recipient.Email,
		"X-SourceControl-Repository":        repo.Name,
		"X-SourceControl-Repository-Path":   repo.FullName(),
		"X-SourceControl-Repository-Link":   repo.HTMLURL(),
		"X-SourceControl-Issue-ID":          strconv.FormatInt(ctx.Issue.Index, 10),
		"X-SourceControl-Issue-Link":        ctx.Issue.HTMLURL(),
