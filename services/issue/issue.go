...
func RebuildPullRequest(ctx context.Context, issue *issues_model.Issue, doer *user_model.User, remoteAddress string) {
	notification.NotifyPullRequestRebuild(ctx, doer, issue, remoteAddress)
}
...
func ChangeTitle(ctx context.Context, issue *issues_model.Issue, doer *user_model.User, title string) (err error) {
	...
	// если эта задача является PR'ом и префикс PR - WIP и новый заголовок !WIP
