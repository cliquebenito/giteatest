...
func SignWikiCommit(ctx context.Context, owner, name, repoWikiPath string, u *user_model.User) (bool, string, *git.Signature, error) {
...
Loop:
	for _, rule := range rules {
		...
		case parentSigned:
			gitRepo, err := git.OpenRepository(ctx, owner, name, repoWikiPath)
			if err != nil {
				return false, "", nil, err
			}
			defer gitRepo.Close()
...
func SignCRUDAction(ctx context.Context, owner, name, repoPath string, u *user_model.User, tmpBasePath, parentCommit string) (bool, string, *git.Signature, error) {
...
Loop:
	for _, rule := range rules {
		...
		case parentSigned:
			gitRepo, err := git.OpenRepository(ctx, owner, name, tmpBasePath)
			if err != nil {
				return false, "", nil, err
			}
			defer gitRepo.Close()
...
Loop:
	for _, rule := range rules {
		...
		case approved:
			protectedBranch, err := git_model.GetMergeMatchProtectedBranchRule(ctx, repo.ID, pr.BaseBranch)
			if err != nil {
				log.Error("Error has occured while get merge match branch with repo id - %d, branch name: %v", repo.ID, pr.BaseBranch, err)
				return false, "", nil, fmt.Errorf("Err: get merge match protected branch: %w", err)
			}
		...
		case baseSigned:
			if gitRepo == nil {
				gitRepo, err = git.OpenRepository(ctx, pr.BaseRepo.OwnerName, pr.BaseRepo.Name, tmpBasePath)
				if err != nil {
					return false, "", nil, err
				}
				defer gitRepo.Close()
		...
		case headSigned:
			if gitRepo == nil {
				gitRepo, err = git.OpenRepository(ctx, pr.HeadRepo.OwnerName, pr.HeadRepo.Name, tmpBasePath)
				if err != nil {
					return false, "", nil, err
				}
				defer gitRepo.Close()
