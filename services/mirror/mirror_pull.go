...
func runSync(ctx context.Context, m *repo_model.Mirror) ([]*mirrorSyncResult, bool) {
	...
	gitRepo, err := git.OpenRepository(ctx, m.Repo.OwnerName, m.Repo.Name, repoPath)
	if err != nil {
	...
	branches, _, err := git.GetBranchesByPath(ctx, m.Repo.OwnerName, m.Repo.Name, m.Repo.RepoPath(), 0, 0)
	if err != nil {
...
func SyncPullMirror(ctx context.Context, repoID int64) bool {
	...
	if len(results) == 0 {
		...
		gitRepo, err = git.OpenRepository(ctx, m.Repo.OwnerName, m.Repo.Name, m.Repo.RepoPath())
		if err != nil {
