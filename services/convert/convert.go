...
func ToBranch(ctx context.Context, repo *repo_model.Repository, b *git.Branch, c *git.Commit, bp *protected_branch.ProtectedBranch, user *user_model.User, isRepoAdmin bool) (*api.Branch, error) {
	if bp == nil {
		var hasPerm bool
		var canPush bool
		var err error
	...
	if user != nil {
		...
		bp.Repo = repo
		branch.UserCanPush = git_model.CanUserPush(db.DefaultContext, *bp, user)
...
func ToBranchProtection(bp *protected_branch.ProtectedBranch) *api.BranchProtection {
	...
	if err != nil {
		log.Error("Error has occured while GetUserNamesByIDs (MergeWhitelistUserIDs): %v", err)
	}
	approvalsWhitelistUsernames, err := user_model.GetUserNamesByIDs(bp.ApprovalsWhitelistUserIDs)
	if err != nil {
		log.Error("Error has occured while GetUserNamesByIDs (ApprovalsWhitelistUserIDs): %v", err)
	}
	...
	return &api.BranchProtection{
		...
		EnablePush:                    !bp.EnableWhitelist || (bp.EnableWhitelist && len(bp.WhitelistUserIDs) == 0),
