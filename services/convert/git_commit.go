...
func ToPayloadCommit(ctx context.Context, repo *repo_model.Repository, c *git.Commit) *api.PayloadCommit {
	...
	if c.Author.Email != "" {
		...
		if author, err := user_model.GetUserByName(ctx, c.Author.Name); err == nil {
			authorUsername = author.Name
		} else if !user_model.IsErrUserNotExist(err) {
			log.Error("ToPayloadCommit user_model.GetUserByName failed while getting user by name: %s:", c.Author.Name)
		}
	}
	...
	if c.Author.Email != "" {
		...
		if commiter, err := user_model.GetUserByName(ctx, c.Committer.Name); err == nil {
			committerUsername = commiter.Name
		} else if !user_model.IsErrUserNotExist(err) {
			log.Error("ToPayloadCommit user_model.GetUserByName failed while getting user by name: %s:", c.Committer.Name)
		}
	}
...
func ToCommit(ctx context.Context, repo *repo_model.Repository, gitRepo *git.Repository, commit *git.Commit, userCache map[string]*user_model.User, opts ToCommitOptions) (*api.Commit, error) {
	...
	if ok {
		...
		if err != nil && !user_model.IsErrUserNotExist(err) {
			author, err = user_model.GetUserByName(ctx, commit.Committer.Name)
			if err != nil && !user_model.IsErrUserNotExist(err) {
				return nil, err
			}
		} else if err == nil {
	...
	if ok {
		...
		if err != nil && !user_model.IsErrUserNotExist(err) {
			committer, err = user_model.GetUserByName(ctx, commit.Committer.Name)
			if err != nil && !user_model.IsErrUserNotExist(err) {
				return nil, err
			}
		} else if err == nil {
