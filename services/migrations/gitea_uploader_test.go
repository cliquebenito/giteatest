...
func TestGiteaUploadUpdateGitForPullRequest(t *testing.T) {
	...
	fromGitRepo, err := git.OpenRepository(git.DefaultContext, fromRepo.OwnerName, fromRepo.Name, fromRepo.RepoPath())
	assert.NoError(t, err)
	...
	forkGitRepo, err := git.OpenRepository(git.DefaultContext, forkRepo.OwnerName, forkRepo.Name, forkRepo.RepoPath())
	assert.NoError(t, err)
