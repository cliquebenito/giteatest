...
function initEditPreviewTab($form) {
  const $tabMenu = $form.find('.tabular.menu, .sc-button-group_tabs');
...
function initImageDiffTabs() {
  const $imageDiff = $('.image-diff');
  if ($imageDiff) {
    const $tabs = $imageDiff.find('.new-menu-inner .item[data-tab]');
    $tabs.on('click', function(event) {
      $tabs.removeClass('active');
      const tab = $(this).attr('data-tab');
      $(this).addClass('active');
      $imageDiff.find(`.tab.image-diff-container`).removeClass('active')
      const $tabContainer = $imageDiff.find(`.tab.image-diff-container[data-tab="${tab}"`);
