...
async function onEditContent(event) {
  ...
  const cancelAndReset = (dz) => {
    ...
    if (comboMarkdownEditor && comboMarkdownEditor.textarea) {
      comboMarkdownEditor.textarea.value = comboMarkdownEditor.originalContent || '';
    } else {
      console.error('comboMarkdownEditor не содержит свойства textarea');
    }
  ...
  if (!$editContentZone.html()) {
    ...
    comboMarkdownEditor.originalContent = comboMarkdownEditor.value();
    ...
    if (!comboMarkdownEditor.originalContent) {
      comboMarkdownEditor.originalContent = comboMarkdownEditor.value();
    }
  }
