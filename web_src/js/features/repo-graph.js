...
export function initRepoGraphGit() {
  ...
  $('#flow-color-monochrome').on('click', () => {
    ...
    $('.pagination a').each((_, that) => {
      const href = DOMPurify.sanitize($(that).attr('href'));
      if (!href) return;
      const url = new URL(href, window.location);
      const params = url.searchParams;
      params.set('mode', 'monochrome');
      url.search = `?${params.toString()}`;
      $(that).attr('href', DOMPurify.sanitize(getSafetyUrl(url.href)));
    });
  });
  ...
  const updateGraph = () => {
    ...
    (async () => {
      const sanitizedAjaxUrl = getSafetyUrl(String(ajaxUrl));
      const div = $(DOMPurify.sanitize(await $.ajax(sanitizedAjaxUrl)));
