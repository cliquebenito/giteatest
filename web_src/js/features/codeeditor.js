...
function togglePreviewDisplay(previewable) {
  ...
  if (previewable) {
    ...
    if (writeTab) {
      writeTab.classList.remove('sc-button-group__item_inactive');
    }
  } else {
    if (writeTab && writeTab.getAttribute('data-new-file') !== 'false') {
      writeTab.classList.add('sc-button-group__item_inactive');
    }
