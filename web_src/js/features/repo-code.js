...
import {getSafetyUrl, toAbsoluteUrl} from '../utils.js';
...
function selectRange($list, $select, $from) {
  ...
  const updateIssueHref = function (anchor) {
    ...
    const urlIssueNew = DOMPurify.sanitize($refInNewIssue.attr('data-url-issue-new'));
    const urlParamBodyLink = DOMPurify.sanitize($refInNewIssue.attr('data-url-param-body-link'));
    const issueContent = `${toAbsoluteUrl(urlParamBodyLink)}#${anchor}`; // the default content for issue body
    const href = getSafetyUrl(`${urlIssueNew}?body=${encodeURIComponent(issueContent)}`)
    $refInNewIssue.attr('href', href);
  };
  ...
  const updateViewGitBlameFragment = function (anchor) {
    ...
    let href = DOMPurify.sanitize($viewGitBlame.attr('href'));
    href = `${href.replace(/#L\d+$|#L\d+-L\d+$/, '')}`;
    if (anchor.length !== 0) {
      href = getSafetyUrl(`${href}#${anchor}`);
    }
    $viewGitBlame.attr('href', getSafetyUrl(href));
  };
  ...
  const updateCopyPermalinkUrl = function(anchor) {
    ...
    let link = getSafetyUrl($copyPermalink.attr('data-url'));
  ...
  const sanitizedRelAttr = DOMPurify.sanitize($select.attr('rel'));
  changeHash(`#${sanitizedRelAttr}`);

  updateIssueHref(sanitizedRelAttr);
  updateViewGitBlameFragment(sanitizedRelAttr);
  updateCopyPermalinkUrl(sanitizedRelAttr);
}
...
export function initRepoCodeView() {
  if ($('.code-view .lines-num').length > 0) {
    $(document).on('click', '.lines-num span', function (e) {
      ...
      const sanitizedId = DOMPurify.sanitize($select.attr('id'))
      selectRange($list, $list.filter(`[rel=${sanitizedId}]`), (e.shiftKey ? $list.filter('.active').eq(0) : null));
