...
import {getSafetyUrl} from "../utils.js";
...
export function initRepoPullRequestReview() {
  if (window.location.hash && window.location.hash.startsWith('#issuecomment-')) {
    ...
    const commentDiv = $(getSafetyUrl(window.location.hash));
    if (commentDiv) {
...
function renderIssuesLinks($issueTitle) {
  const { csrfToken, baseLink, issuePullId: issueId }  = window.config;
  const { globalFetch } = window;
  const issueTitle = $issueTitle.text();
  let htmlStr = issueTitle;
    ...
    .then((res) => {
      ...
      if (res && Array.isArray(res.units)) {
        res.units.forEach(unit => {
          htmlStr = replaceWord(htmlStr, unit.code, `<a href="${unit.url}" target="_blank">${unit.code}</a>`)
        })
        $issueTitle.html(htmlStr);
      }
    }).catch((err) => {
...
function replaceWord(text, searchWord, replaceWord) {
  let regex = new RegExp(`\\b${searchWord}\\b`, 'g');
