      ...
      <div class="sc-box sc-box_attached">
        <h4 class="sc-box__header">
          <div class="gt-f1 gt-df gt-ac">
            {{ textMyRepos }}
            <span class="sc-badge sc-badge_info gt-ml-3">{{ reposTotalCount }}</span>
          </div>
          <a v-if="!enableRoleModel" :href="subUrl + '/repo/create' + (isOrganization ? '?org=' + organizationId : '')" :data-tooltip-content="textNewRepo">
            <svg-icon name="octicon-plus"/>
            <span class="sr-only">{{ textNewRepo }}</span>
          </a>
        </h4>
        <div class="sc-box__body repos-search">
          ...
          <div class="sc-tabs-nav sc-tabs-nav_small">
            <a class="sc-tabs-nav__item" :class="{'sc-tabs-nav__item_active': reposFilter === 'all'}" @click="changeReposFilter('all')">
              {{ textAll }}
              <div v-show="reposFilter === 'all'" class="sc-badge sc-badge_info">{{ repoTypeCount }}</div>
            </a>
            <a class="sc-tabs-nav__item" :class="{'sc-tabs-nav__item_active': reposFilter === 'sources'}" @click="changeReposFilter('sources')">
              {{ textSources }}
              <div v-show="reposFilter === 'sources'" class="sc-badge sc-badge_info">{{ repoTypeCount }}</div>
            </a>
            <a class="sc-tabs-nav__item" :class="{'sc-tabs-nav__item_active': reposFilter === 'forks'}" @click="changeReposFilter('forks')">
              {{ textForks }}
              <div v-show="reposFilter === 'forks'" class="sc-badge sc-badge_info">{{ repoTypeCount }}</div>
            </a>
            <a class="sc-tabs-nav__item" :class="{'sc-tabs-nav__item_active': reposFilter === 'mirrors'}" @click="changeReposFilter('mirrors')" v-if="isMirrorsEnabled">
              {{ textMirrors }}
              <div v-show="reposFilter === 'mirrors'" class="sc-badge sc-badge_info">{{ repoTypeCount }}</div>
            </a>
            <a class="sc-tabs-nav__item" :class="{'sc-tabs-nav__item_active': reposFilter === 'collaborative'}" @click="changeReposFilter('collaborative')">
              {{ textCollaborative }}
              <div v-show="reposFilter === 'collaborative'" class="sc-badge sc-badge_info">{{ repoTypeCount }}</div>
            </a>
          </div>
        </div>
        <div v-if="repos.length" class="sc-box__body table">
    ...
    <div v-if="!isOrganization" v-show="tab === 'organizations'" class="ui tab active list dashboard-orgs">
      <div class="sc-box sc-box_attached">
        <h4 class="sc-box__header">
          <div class="gt-f1 gt-df gt-ac">
            {{ textMyOrgs }}
            <span class="sc-badge sc-badge_info gt-ml-3">{{ organizationsTotalCount }}</span>
          </div>
        ...
        <div v-if="organizations.length" class="sc-box__body table">
          <ul class="repo-owner-name-list">
            <li v-for="org in organizations" :key="org.name">
              <a class="repo-list-link gt-df gt-ac gt-sb" :href="subUrl + '/' + encodeURIComponent(org.name)">
                <div class="text truncate item-name gt-f1">
                  ...
                  <span class="sc-badge sc-badge_outlined gt-ml-2" v-if="org.org_visibility !== 'public'">
...
const {appSubUrl, assetUrlPrefix, pageData, enableRoleModel} = window.config;

const commitStatus = {
  pending: {name: 'octicon-dot-fill', color: 'grey'},
  running: {name: 'octicon-dot-fill', color: 'yellow'},
  success: {name: 'octicon-check', color: 'green'},
  error: {name: 'octicon-dot-fill', color: 'red'},
  failure: {name: 'octicon-x', color: 'red'},
  warning: {name: 'octicon-dot-fill', color: 'yellow'},
};
