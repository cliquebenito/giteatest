<template>
  <div class="ui floating filter dropdown custom">
    <button class="branch-dropdown-button sc-button sc-button_small" @click="menuVisible = !menuVisible" @keyup.enter="menuVisible = !menuVisible">
      ...
      <template v-else>
        ...
        <span ref="dropdownRefName">{{ refNameText }}</span>
      </template>
      <span class="sc-button__icon">
        <svg-icon name="octicon-triangle-down" :size="14" class-name="dropdown icon"/>
      </span>
    </button>
    <div class="sc-dropdown-menu menu transition sc-dropdown-overflow-hidden" :class="{visible: menuVisible}" v-if="menuVisible" v-cloak>
      ...
      <div class="scrolling menu" ref="scrollContainer">
        ...
        <div class="item" v-if="showCreateNewBranch && hasPrivileges" :class="{active: active === filteredItems.length}" :ref="'listItem' + filteredItems.length">
          <a href="#" class="create-new-item" @click="createNewBranch()">
        ...
        <div class="message" v-else-if="searchTerm && showNoResults">
          {{ noResults }}
        </div>
      </div>
    </div>
...
export function initRepoBranchTagSelector(selector) {
  for (const [elIndex, elRoot] of document.querySelectorAll(selector).entries()) {
    const data = {
      ...
      hasPrivileges: window.config.privilegesCreate || window.config.customPrivilegesCreateBranch,
      items: [],
      searchTerm: '',
      refNameText: '',
      menuVisible: false,
...
<style scoped>
.menu .item a:not(.create-new-item) {
