...
const entry = {
  ...
  'home-page': [
    fileURLToPath(new URL('web_src/css/sc/pages/home.css', import.meta.url)),
  ],
  'error-page': [
    fileURLToPath(new URL('web_src/css/sc/error-page.css', import.meta.url)),
  ],
  ...themes,
};
...
export default {
  ...
  output: {
    ...
    chunkFilename: ({ chunk }) => {
      const language = (/monaco.*languages?_.+?_(.+?)_/.exec(chunk.id) ||
        [])[1];
      return `js/${
        language ? `monaco-language-${language.toLowerCase()}` : `[name]`
      }.[contenthash:8].js`;
    },
  },
  optimization: {
    ...
    isProduction
      ? new LicenseCheckerWebpackPlugin({
          ...
          outputWriter: ({ dependencies }) => {
            ...
            const goModules = JSON.parse(goJson).map(
              ({ name, licenseText }) => {
                return { name, body: formatLicenseText(licenseText) };
              }
            );
            const jsModules = dependencies.map(
              ({ name, version, licenseName, licenseText }) => {
                return {
                  name,
                  version,
                  licenseName,
                  body: formatLicenseText(licenseText),
                };
              }
            );

            const modules = [...goModules, ...jsModules].sort((a, b) =>
              a.name.localeCompare(b.name)
            );
            return modules
              .map(({ name, version, licenseName, body }) => {
                const title = licenseName
          ...
          override: {
            '@sbt-works/side-menu': { licenseName: 'Unlicense' }, // у пакетов не заполнен ключ license в package.json
            'jquery.are-you-sure@*': { licenseName: 'MIT' }, // https://github.com/codedance/jquery.AreYouSure/pull/147
            'khroma@*': { licenseName: 'MIT' }, // https://github.com/fabiospampinato/khroma/pull/33
          },
          emitError: true,
          allow:
            '(Apache-2.0 OR BSD-2-Clause OR BSD-3-Clause OR MIT OR ISC OR CPAL-1.0 OR Unlicense OR EPL-1.0 OR EPL-2.0)',
        })
      : new AddAssetPlugin(
          'js/licenses.txt',
          `Licenses are disabled during development`
        ),
  ],
  performance: {
    hints: false,
