...
func TestCantMergeConflict(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, giteaURL *url.URL) {
		...
		gitRepo, err := git.OpenRepository(git.DefaultContext, user1.Name, repo1.Name, repo_model.RepoPath(user1.Name, repo1.Name))
		assert.NoError(t, err)
...
func TestCantMergeUnrelated(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, giteaURL *url.URL) {
		...
		gitRepo, err := git.OpenRepository(git.DefaultContext, "user1", "repo1", path)
		assert.NoError(t, err)
		pr := unittest.AssertExistsAndLoadBean(t, &issues_model.PullRequest{
			HeadRepoID: repo1.ID,
			BaseRepoID: repo1.ID,
