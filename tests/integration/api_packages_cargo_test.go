...
func testPackageCargo(t *testing.T, _ *neturl.URL) {
	...
	packageHomepage := "https://sourcecontrol.sbertech.ru/"
	packageLicense := "MIT"
   ...
   "deps":[
      {
         "name":"dep",
         "version_req":"1.0",
         "registry": "https://sourcecontrol.sbertech.ru/user/_cargo-index",
         "kind": "normal",
   ...
   "license":"` + packageLicense + `"
	...
	readGitContent := func(t *testing.T, path string) string {
		gitRepo, err := git.OpenRepository(db.DefaultContext, repo.OwnerName, repo.Name, repo.RepoPath())
		assert.NoError(t, err)
		defer gitRepo.Close()
			...
			t.Run("Index", func(t *testing.T) {
				t.Run("Git", func(t *testing.T) {
					t.Run("Entry", func(t *testing.T) {
						...
						assert.Equal(t, "https://sourcecontrol.sbertech.ru/user/_cargo-index", *dep.Registry)
