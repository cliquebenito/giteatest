...
func IsErrLastOrgOwner(err error) bool {
	errLastOrgOwner := new(ErrLastOrgOwner)
...
// IsVisibilityLimited проверка на тип проекта ограниченный
func (o *Organization) IsVisibilityLimited() bool {
	return o.Visibility == structs.VisibleTypeLimited
}
...
func GetAllActiveOrganization(ctx context.Context) (allOrg []*Organization, err error) {
	// Находим список организаций.
	if err = db.GetEngine(ctx).
		Where("is_active").
		And("type = ?", user_model.UserTypeOrganization).
		Find(&allOrg); err != nil {
		return nil, errors.New("failed get all active organizations")
	}
	return
}
...
// GetOrganizationByIDs получаес список оргагизация по массиву organization_id
func GetOrganizationByIDs(ctx context.Context, organizationIDs []int64) ([]*Organization, error) {
	var organizations []*Organization
	if err := db.GetEngine(ctx).Table("user").Where(builder.In("id", organizationIDs)).Find(&organizations); err != nil {
		log.Error("Err: get organization by id: %v", err)
		return nil, ErrOrgNotExist{ID: organizationIDs[0]}
	}
...
func CreateRelationTenantOrganization(tenantProject *tenant_model.ScTenantOrganizations) error {
	if err := tenant_model.InsertTenantOrganization(db.DefaultContext, tenantProject); err != nil {
		log.Error("Error inserting tenant organization: %v", err)
		return fmt.Errorf("CreateRelationTenantOrganization failed: %w", err)
	}
	return nil
}

// RemoveRelationTenantOrganization удаляем
func RemoveRelationTenantOrganization(ctx context.Context, orgID int64, tenantID string) error {
	if _, err := db.GetEngine(ctx).Delete(tenant_model.ScTenantOrganizations{TenantID: tenantID, OrganizationID: orgID}); err != nil {
		log.Error("Error deleting tenant organization: %v", err)
		return fmt.Errorf("RemoveRelationTenantOrganization failed: %w", err)
	}
	return nil
}
