...
func applyConditions(sess *xorm.Session, opts *IssuesOptions) *xorm.Session {
	...
	if opts.SubscriberID > 0 && !setting.SourceControl.TenantWithRoleModeEnabled {
	...
	if setting.SourceControl.TenantWithRoleModeEnabled {
		sess.Where(builder.In("issue.repo_id", opts.RepoIDs))
	} else {
		if opts.User != nil {
...
func GetRepoIDsForIssuesOptions(opts *IssuesOptions, user *user_model.User) ([]int64, error) {
	...
	if setting.SourceControl.TenantWithRoleModeEnabled {
		sess.Where(builder.In("repo_id", opts.RepoIDs))
	}
