...
const (
	...
	ReviewTypeRequest
	// ReviewTypeInactive inactive review
	ReviewTypeInactive
)
...
func IsOfficialReviewer(ctx context.Context, issue *Issue, reviewers ...*user_model.User) (bool, error) {
	...
	rule, err := git_model.GetMergeMatchProtectedBranchRule(ctx, pr.BaseRepoID, pr.BaseBranch)
	if err != nil {
		log.Error("Err: get merge math protected branch with base repo id - %d, base branch - %s: %v", pr.BaseRepoID, pr.BaseBranch, err)
		return false, fmt.Errorf("Err: get merge math protected branch with base repo id - %d, base branch - %s: %w", pr.BaseRepoID, pr.BaseBranch, err)
	}
	if rule == nil {
		return false, nil
	}
	...
	for _, reviewer := range reviewers {
		official := git_model.IsUserOfficialReviewer(ctx, *rule, reviewer)
		if official {
			return official, nil
		}
	}
...
func SubmitReview(doer *user_model.User, issue *Issue, reviewType ReviewType, content, commitID string, stale bool, attachmentUUIDs []string) (*Review, *Comment, error) {
	...
	if err != nil {
		...
		if err := UpdateStatusReviewCodeOwners(ctx, reviewType, issue, doer); err != nil {
			return nil, nil, fmt.Errorf("update status review code owners: %w", err)
		}
...
func AddReviewRequest(ctx context.Context, issue *Issue, reviewer, doer *user_model.User) (*Comment, error) {
	...
	if _, err := sess.Table(new(repo.CodeOwners)).
		Where(builder.Eq{
			"issue_id": issue.ID,
			"owner_id": reviewer.ID,
		}).
		Update(map[string]interface{}{
			"approval_status": ReviewTypeRequest,
		}); err != nil {
		return nil, fmt.Errorf("update code_owners: %w", err)
	}
...
func RemoveReviewRequest(issue *Issue, reviewer, doer *user_model.User) (*Comment, error) {
	...
	review, err := GetReviewByIssueIDAndUserID(ctx, issue.ID, reviewer.ID)
	if err != nil && !IsErrReviewNotExist(err) {
		log.Error("Error has occurred while getting review. Error: %v", err)
		return nil, fmt.Errorf("get review: %w", err)
	}

	_, err = db.GetEngine(ctx).Table(new(Review)).
		Where(builder.Eq{
			"issue_id":    issue.ID,
			"reviewer_id": reviewer.ID,
		}).
		Update(map[string]interface{}{
			"type": ReviewTypeInactive,
		})
	if err != nil {
		log.Error("Error has occurred while updating review type. Error: %v", err)
	...
	if err != nil {
		log.Error("Error has occurred while getting official reviewer. Error: %v", err)
		return nil, fmt.Errorf("get official reviewer: %w", err)
	} else if official {
		if err := restoreLatestOfficialReview(ctx, issue.ID, reviewer.ID); err != nil {
			log.Error("Error has occurred while restoring latest official review. Error: %v", err)
			return nil, fmt.Errorf("restore latest official review: %w", err)
		}
	}
