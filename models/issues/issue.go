...
// GetRepositoryByIssueID получаем репозиторий по issue_id
func GetRepositoryByIssueID(ctx context.Context, issueID int64) (*repo_model.Repository, error) {
	var repoID int64
	if _, err := db.GetEngine(ctx).
		Select("issue.repo_id").
		Table("issue").
		Where(builder.Eq{"issue.id": issueID}).Get(&repoID); err != nil {
		log.Error("Error has occurred while getting repository by issue_id: %d", issueID)
		return nil, fmt.Errorf("getting repository id: %w", err)
	}

	repo := new(repo_model.Repository)
	if has, err := db.GetEngine(ctx).ID(repoID).Get(repo); err != nil {
		return nil, fmt.Errorf("getting repository: %w", err)
	} else if !has {
		log.Error("Error has occurred while getting repository by repoID: %d", repoID)
		return nil, fmt.Errorf("repository not found for issue %d", issueID)
	}
