...
func (c *Comment) LoadPushCommits(ctx context.Context) (err error) {
	...
	if c.IsForcePush {
		...
		gitRepo, closer, err := git.RepositoryFromContextOrOpen(ctx, c.Issue.Repo.OwnerName, c.Issue.Repo.Name, repoPath)
		if err != nil {
			return err
		}
		defer closer.Close()
...
// PosterIDString converts poster ID to string
func (c *Comment) PosterIDString() string {
	return strconv.FormatInt(c.PosterID, 10)
}
