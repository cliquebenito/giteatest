...
type ErrLockMigration struct {
	Version int64
}

// IsErrLockMigration checks if an error is a ErrLockMigration.
func IsErrLockMigration(err error) bool {
	_, ok := err.(ErrLockMigration)
	return ok
}

func (err ErrLockMigration) Error() string {
	msg := fmt.Sprintf("Your database is already migrating to migration version: %d. Please wait", err.Version)
	if !setting.IsProd {
		msg += fmt.Sprintf("\nIf you are in development and really know what you're doing, you can unlocking migration by executing: UPDATE change_lock SET locked=false WHERE id=1;")
	}
	return msg
}
