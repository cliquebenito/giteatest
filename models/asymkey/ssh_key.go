...
func GetSSHKeyByUserAndTitle(userID int64, title string) (*PublicKey, error) {
	key := new(PublicKey)
	has, err := db.GetEngine(db.DefaultContext).
		Where(builder.Eq{"owner_id": userID}).
		And(builder.Eq{"name": title}).
		Get(key)
	if err != nil {
		return nil, err
	} else if !has {
		return nil, ErrKeyNotExist{Title: title}
	}
	return key, nil
}
