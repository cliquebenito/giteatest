...
type ErrInvalidFormatOrLength struct{}

func NewErrInvalidFormatOrLength() *ErrInvalidFormatOrLength {
	return &ErrInvalidFormatOrLength{}
}

func IsErrInvalidFormatOrLength(err error) bool {
	var customErr *ErrInvalidFormatOrLength
	return errors.As(err, &customErr)
}

func (*ErrInvalidFormatOrLength) Error() string {
	return "не верный формат/длина ssh-key, введите корректный ssh-key"
}
...
func NewErrInvalidKeyFormat(length int) *ErrInvalidKeyFormat {
	return &ErrInvalidKeyFormat{length}
}

func IsErrInvalidKeyFormat(err error) bool {
	var customErr *ErrInvalidKeyFormat
	return errors.As(err, &customErr)
}

func (e *ErrInvalidKeyFormat) Error() string {
	return fmt.Errorf("invalid key format: not enough length %d", e.Length).Error()
}

type ErrInvalidKeyLine struct {
	KeyLine string
}

func NewErrInvalidKeyLine(keyLine string) *ErrInvalidKeyLine {
	return &ErrInvalidKeyLine{keyLine}
}

func IsErrInvalidKeyLine(err error) bool {
	var customErr *ErrInvalidKeyLine
	return errors.As(err, &customErr)
}

func (e *ErrInvalidKeyLine) Error() string {
	return fmt.Errorf("not enough fields in public key line: %s", e.KeyLine).Error()
}
...
func NewErrInvalidKeyType(keyType string) *ErrInvalidKeyType {
	return &ErrInvalidKeyType{keyType}
}

func IsErrInvalidKeyType(err error) bool {
	var customErr *ErrInvalidKeyType
	return errors.As(err, &customErr)
}

func (e *ErrInvalidKeyType) Error() string {
	return fmt.Errorf("unsupported key length detection for type: %s", e.KeyType).Error()
}
...
func NewErrInvalidKeyLength(length, minLen int) *ErrInvalidKeyLength {
	return &ErrInvalidKeyLength{length, minLen}
}

func IsErrInvalidKeyLength(err error) bool {
	var customErr *ErrInvalidKeyLength
	return errors.As(err, &customErr)
}

func (e *ErrInvalidKeyLength) Error() string {
	return fmt.Errorf("key length is not enough: got %d, needs %d", e.Length, e.MinLen).Error()
}

type ErrNotAllowedKeyType struct {
	KeyType string
}

func NewErrNotAllowedKeyType(keyType string) *ErrNotAllowedKeyType {
	return &ErrNotAllowedKeyType{keyType}
}

func IsErrNotAllowedKeyType(err error) bool {
	var customErr *ErrNotAllowedKeyType
	return errors.As(err, &customErr)
}

func (e *ErrNotAllowedKeyType) Error() string {
	return fmt.Errorf("key type is not allowed: %s", e.KeyType).Error()
}

type ErrInvalidPEMBlock struct{}

func NewErrInvalidPEMBlock() *ErrInvalidPEMBlock {
	return &ErrInvalidPEMBlock{}
}

func IsErrInvalidPEMBlock(err error) bool {
	var customErr *ErrInvalidPEMBlock
	return errors.As(err, &customErr)
}

func (e *ErrInvalidPEMBlock) Error() string {
	return fmt.Errorf("failed to parse PEM block containing the public key").Error()
}
