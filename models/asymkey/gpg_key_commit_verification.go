...
func ParseCommitWithSignature(ctx context.Context, c *git.Commit) *CommitVerification {
	...
	if c.Committer != nil {
		...
		if c.Committer.Email != "" {
			...
			committer, err = user_model.GetUserByName(ctx, c.Committer.Name)
		}
		if err != nil { // Skipping not user for committer
			...
			if !user_model.IsErrUserNotExist(err) {
				log.Error("ParseCommitWithSignature failed GetUserByEmail or GetUserByName while getting user: %v", err)
