...
func extractTypeFromBase64Key(key string) (string, error) {
	...
	if err != nil || len(b) < 4 {
		return "", NewErrInvalidFormatOrLength()
	}

	keyLength := int(binary.BigEndian.Uint32(b))
	if len(b) < 4+keyLength {
		return "", NewErrInvalidKeyFormat(keyLength)
	}
...
func parseKeyString(content string) (string, error) {
	...
	if strings.HasPrefix(content, ssh2keyStart) {
		...
		if len(keyType) == 0 {
			...
			return "", NewErrNotAllowedKeyType(keyType)
		}
	}
...
func CheckPublicKeyString(content string) (_ string, err error) {
	...
	if minLen, found := setting.SSH.MinimumKeySizes[keyType]; found && length >= minLen {
		...
		return "", NewErrInvalidKeyLength(length, minLen)
	}
	return "", NewErrNotAllowedKeyType(keyType)
}
...
func SSHNativeParsePublicKey(keyLine string) (string, int, error) {
	...
	if len(fields) < 2 {
		return "", 0, NewErrInvalidKeyLine(keyLine)
	}

	raw, err := base64.StdEncoding.DecodeString(fields[1])
	if err != nil {
		return "", 0, NewErrInvalidFormatOrLength()
	}
	...
	return "", 0, NewErrInvalidKeyType(pkey.Type())
}
