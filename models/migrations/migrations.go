...
import (
	...
	"code.gitea.io/gitea/models/migrations/v1_28"
	"code.gitea.io/gitea/models/migrations/v1_29"
	"code.gitea.io/gitea/models/migrations/v1_30"
	"code.gitea.io/gitea/models/migrations/v1_31"
	"code.gitea.io/gitea/models/migrations/v1_33"
	"code.gitea.io/gitea/models/migrations/v1_6"
	"code.gitea.io/gitea/models/migrations/v1_7"
	"code.gitea.io/gitea/models/migrations/v1_8"
	"code.gitea.io/gitea/models/migrations/v1_9"
	"code.gitea.io/gitea/models/migrations/v_32"
...
// ChangeLock describes the change_lock table. Should have only one row with id==1
type ChangeLock struct {
	ID          int64 `xorm:"pk default 1"`
	Locked      bool
	LockGranted timeutil.TimeStamp
}

// lock Update row in the change_lock table for locking other migrate of database
func (l *ChangeLock) lock(x *xorm.Engine) error {
	l.Locked = true
	l.LockGranted = timeutil.TimeStampNow()
	count, err := x.ID(1).Where(builder.Eq{"locked": false}).AllCols().Update(l)
	if err != nil {
		return fmt.Errorf("lock: %w", err)
	}
	if count == 0 {
		return models.ErrLockMigration{
...
// unlock Update row in the change_lock table for unlocking other migrate of database
func (l *ChangeLock) unlock(x *xorm.Engine) error {
	l.Locked = false
	if _, err := x.ID(1).AllCols().Update(l); err != nil {
...
var migrations = []Migration{
	...
	// v258 -> v259
	NewMigration("Create table gv_early_adopter", v1_21.CreateEarlyAdopterTable),
	// v259 -> v260
	NewMigration("Create table gv_possible_user", v1_21.CreatePossibleUserTable),
	// v260 -> v261
	NewMigration("Create table sc_sonar_settings", v1_21.CreateScSonarSettingsTable),
	// v261 -> v262
	NewMigration("Create table sc_tenant", v1_22.CreateTenantTable),
	// v262 -> v263
	NewMigration("Create table sc_tenant_organizations", v1_22.CreateTenantOrganizationTable),
	// v263 -> v264
	NewMigration("Change organization visibility", v1_22.ChangeOrganizationVisibility),
	// v264 -> v265
	NewMigration("Update table sc_sonar_settings(add url field)", v1_23.UpdateScSonarSettingsTable),
	// v265 -> v266
	NewMigration("Create table sc_external_hook", v1_24.CreateScExternalHookTable),
	// v266 -> v267
	NewMigration("Create table sc_repo_licenses", v1_24.CreateScRepoLicenses),
	// v267 -> v268
	NewMigration("Create table sc_project_status", v1_25.CreateScSonarProjectStatus),
	// v268 -> v269
	NewMigration("Create table sc_project_metrics", v1_25.CreateScSonarProjectMetrics),
	// v269 -> v270
	NewMigration("Add new column protected_branch enable_sonar_qube", v1_25.AddEnableSonarQube),
	//v270 -> v271
	NewMigration("Update table sc_tenant(add org_key field), if sourcecontrol.enabled = true", v1_25.UpdateScTenantTable),
	//v271 -> v272
	NewMigration("Update table sc_tenant_organizations(add org_key, project_key field), if sourcecontrol.enabled = true", v1_25.UpdateScTenantOrganizationTable),
	// v272 -> v273
	NewMigration("Create table unit_links", v1_26.CreateUnitLinks),
	// v273-> v274
	NewMigration("Create table unit_links_sender_tasks", v1_26.CreateUnitLinksSenderTasks),
	// v274 -> v275
	NewMigration("Create table sc_team_custom_privileges", v1_26.CreateScTeamCustomPrivileges),
	// v275 -> v276
	NewMigration("Create table sc_repo_key", v1_27.CreateScRepoKeyTable),
	// v276 -> v277
	NewMigration("Create table code_owners_settings", v1_28.CreateCodeOwnersSettings),
	// v277 -> v278
	NewMigration("Create table code_owners", v1_28.CreateCodeOwners),
	// v278 -> v279
	NewMigration("Create table code_hub_counter", v1_29.CreateCodeHubCounterTable),
	// v279 -> v280
	NewMigration("Create table code_hub_unique_usages", v1_29.CreateCodeHubUniqueUsagesTable),
	// v280 -> v281
	NewMigration("Create table code_hub_counter_tasks", v1_29.CreateCodeHubCounterTasksTable),
	// v281 -> v282
	NewMigration("Create table repo_marks", v1_30.CreateRepoMarksTable),
	// v282 -> v283
	NewMigration("Change code_hub_counter table schema and name", v1_31.ChangeCodeHubCounterTable),
	// v283 -> 284
	NewMigration("Create table external_metric_counter", v1_31.CreateExternalCounterTable),
	// v284 -> v285
	NewMigration("Change table protected_branch", v1_31.ChangeProtectedBranch),
	// v285 -> 286
	NewMigration("Create table sc_custom_privileges_group", v_32.CreateScCustomPrivileges),
	// 286 -> 287
	NewMigration("Create review_settings and default_reviewers tables", v1_33.CreateReviewSettingsAndDefaultReviewersTable),
}
...
func EnsureUpToDate(x *xorm.Engine) error {
	...
	if minDBVersion > currentDB {
		return fmt.Errorf("DB version %d (<= %d) is too old for auto-migration. Upgrade to SourceControl 1.6.4 first then upgrade to this version", currentDB, minDBVersion)
	}
...
// lockMigration if the database is not locked for migration, create a lock before starting the database migration
func lockMigration(x *xorm.Engine) (*ChangeLock, error) {
	changeLock := &ChangeLock{ID: 1}
	has, err := x.Get(changeLock)
	if err != nil {
		return nil, fmt.Errorf("get: %w", err)
	} else if !has {
		// If the version record does not exist we think
		// it is a fresh installation and we can skip lock.
		changeLock.LockGranted = timeutil.TimeStampNow()
		changeLock.Locked = false

		if _, err = x.InsertOne(changeLock); err != nil {
			return nil, fmt.Errorf("insert: %w", err)
		}
	}

	if changeLock.Locked {
		err = &models.ErrLockMigration{
			Version: int64(minDBVersion + len(migrations)),
		}
		return nil, err
	}

	return changeLock, changeLock.lock(x)
}
...
func Migrate(x *xorm.Engine) error {
	...
	if err := x.Sync(new(ChangeLock)); err != nil {
		return fmt.Errorf("sync: %w", err)
	}
	...
	if minDBVersion > v {
		log.Fatal(setting.AppName + ` no longer supports auto-migration from your previously installed version.
Please try upgrading to a lower version first (suggested v1.6.4), then upgrade to this version.`)
	...
	if int(v-minDBVersion) > len(migrations) {
		msg := fmt.Sprintf("Your database (migration version: %d) is for a newer %s, you can not use the newer database for this old %s release (%d).", v, setting.AppName, setting.AppName, minDBVersion+len(migrations))
		msg += fmt.Sprintf("\n%s will exit to keep your database safe and unchanged. Please use the correct %s release, do not change the migration version manually (incorrect manual operation may lose data).", setting.AppName, setting.AppName)
		if !setting.IsProd {
	...
	neededMigrations := migrations[v-minDBVersion:]
	if len(neededMigrations) == 0 {
		return nil
	}

	changeLock, err := lockMigration(x)
	if err != nil {
		return err
	}
