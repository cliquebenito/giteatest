...
func AddTeamMember(team *organization.Team, userID int64) error {
	ctx, _ := context.WithTimeout(db.DefaultContext, duration)
	...
	if isMember {
		log.Info("Member already in team")
		return nil
	}
	...
	isMember, err = organization.IsTeamMember(txCtx, team.OrgID, team.ID, userID)
	if err != nil {
		return err
	}
	if isMember {
	...
	if err := insertTeamUser(txCtx, team, userID); err != nil {
		return err
	}
	if err := updateUserAccesses(txCtx, team, userID); err != nil {
		return err
	}
...
func insertTeamUser(ctx context.Context, team *organization.Team, userID int64) error {
	sess := db.GetEngine(ctx)
...
func updateUserAccesses(ctx context.Context, team *organization.Team, userID int64) error {
	sess := db.GetEngine(ctx)
	...
	if err := sess.SQL(subQuery.And(builder.NotIn("repo_id", accessSubQuery))).Find(&repoIDs); err != nil {
		return fmt.Errorf("select missing accesses: %w", err)
	}

	batch := make([]*access_model.Access, 0, 100)
	for i, repoID := range repoIDs {
		batch = append(batch, &access_model.Access{RepoID: repoID, UserID: userID, Mode: team.AccessMode})
		if (i%100 == 0 || i == len(repoIDs)-1) && len(batch) > 0 {
