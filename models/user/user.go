...
const DefaultEmail = "source_control@sbertech.ru"

func (u *User) GetDefaultEmail() string {
	if u.Email == "" {
		return DefaultEmail
	}
	return u.Email
...
// IsIAM returns true if user login type is LoginIAM.
func (u *User) IsIAM() bool {
	return u.LoginType == auth.IAM
}
...
// IsLoginNameExist checks if given user login name exist,
func IsLoginNameExist(ctx context.Context, loginName string) (bool, error) {
	if len(loginName) == 0 {
		return false, nil
	}
	// Приводим логин к нижнему регистру
	loginNameLower := strings.ToLower(loginName)
	has, err := db.GetEngine(ctx).
		Where("LOWER(login_name) = ?", loginNameLower).
		Get(&User{})
	if err != nil {
		return false, err
	}
...
func CreateUser(u *User, overwriteDefault ...*CreateUserOverwriteOptions) (err error) {
	...
	isExist, err = IsLoginNameExist(ctx, u.LoginName)
	if err != nil {
		return err
	} else if isExist {
		if u.LoginName != "" {
			return ErrUserLoginNameAlreadyUsed{
...
func CountUsers(opts *CountUserFilter) int64 {
	count, _ := countUsers(db.DefaultContext, opts)
	return count
}

// CountUsersCtx returns number of users.
func CountUsersCtx(ctx context.Context, opts *CountUserFilter) (int64, error) {
	return countUsers(ctx, opts)
}

func countUsers(ctx context.Context, opts *CountUserFilter) (int64, error) {
...
func UpdateUser(ctx context.Context, u *User, changePrimaryEmail bool, cols ...string) error {
	...
	if changePrimaryEmail && u.Email != "" {
	...
	} else if !u.IsOrganization() && u.Email != "" { // check if primary email in email_address table
		...
		if _, err := e.Delete(&EmailAddress{UID: u.ID}); err != nil {
			return err
		}
	}
...
func GetUserByChartName(ctx context.Context, name string) (users []*User, err error) {
	if err = db.GetEngine(ctx).
		Where(builder.Or(
			builder.Like{"LOWER(lower_name)", name},
			builder.Like{"LOWER(full_name)", name},
		)).
		And(
			builder.Eq{"type": UserTypeIndividual},
			builder.Like{"LOWER(email)", "%"},
		).
		Limit(50).
		Find(&users); err != nil {
		return
...
// GetUserByLoginName returns user by given name.
func GetUserByLoginName(ctx context.Context, loginName string) (*User, error) {
	if len(loginName) == 0 {
		return nil, fmt.Errorf("%w", ErrLoginNameNotExist{loginName})
	}
	u := &User{LoginName: strings.ToLower(loginName), Type: UserTypeIndividual}
	has, err := db.GetEngine(ctx).Get(u)
	if err != nil {
		return nil, err
	} else if !has {
...
// Deprecated: use UserDB.GetUserNamesByIDs
...
func ValidateCommitWithEmail(ctx context.Context, c *git.Commit) *User {
	...
	if c.Author.Email != "" {
		...
		u, err = GetUserByName(ctx, c.Author.Name)
	}
	if err != nil {
		return nil
	}
...
func ValidateCommitsWithEmails(ctx context.Context, oldCommits []*git.Commit) []*UserCommit {
	...
	for _, c := range oldCommits {
		var u *User
		if c.Author != nil {
			if v, ok := emails[c.Author.Email]; !ok && c.Author.Email != "" {
				u, _ = GetUserByEmail(ctx, c.Author.Email)
				emails[c.Author.Email] = u
			} else if c.Author.Name != "" {
				u, _ = GetUserByName(ctx, c.Author.Name)
				emails[c.Author.Email] = u
			} else {
				u = v
			}
