...
// ErrUserLoginNameAlreadyUsed represents a "ErrUserLoginNameAlreadyUsed" kind of error
type ErrUserLoginNameAlreadyUsed struct {
	LoginName string
}

// Реализация интерфейса error
func (err ErrUserLoginNameAlreadyUsed) Error() string {
	return fmt.Sprintf("user key already exists [user_key: %s]", err.LoginName)
}

// Unwrap возвращает базовую ошибку для сравнения через errors.Is
func (err ErrUserLoginNameAlreadyUsed) Unwrap() error {
	return errors.New("user login name already used")
}

// IsLoginNameAlreadyUsed проверяет, является ли ошибка ErrUserLoginNameAlreadyUsed
func IsLoginNameAlreadyUsed(err error) bool {
	return errors.As(err, &ErrUserLoginNameAlreadyUsed{})
}

// ErrLoginNameNotExist represents a "LoginNameNotExist" kind of error.
type ErrLoginNameNotExist struct {
	LoginName string
}

// IsErrLoginNameNotExist checks if an error is a ErrLoginNameNotExist.
func IsErrLoginNameNotExist(err error) bool {
	return errors.As(err, &ErrLoginNameNotExist{})
}

func (err ErrLoginNameNotExist) Error() string {
	return fmt.Sprintf("user key does not exist [user_key: %s]", err.LoginName)
}

// Unwrap unwraps this error as a ErrLoginNameNotExist error
func (err ErrLoginNameNotExist) Unwrap() error {
	return util.ErrNotExist
}
