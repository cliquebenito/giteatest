...
// GetUserMap метод для получения мапы пользователе
func (users UserList) GetUserMap() map[int64]*User {
	results := make(map[int64]*User, len(users))
	for _, user := range users {
...
// GetUsersNotInUserIDs получаем пользователей, которые не относятся не к одному тенанту
func GetUsersNotInUserIDs(ids []int64) (UserList, error) {
	var users []*User
	err := db.GetEngine(db.DefaultContext).
		Where(builder.NotIn("id", ids)).
		Find(&users)
	if err != nil {
		log.Error("GetUsersNotInUserIDs failed: %v", err)
		return nil, err
	}
	return users, nil
}
