...
type GetFeedsOptions struct {
	...
	OrganizationIDs []int64                // the organizations id in tenant
	AllowedRepoIDs  []int64                // the allowed repos id
}
...
func activityQueryCondition(opts GetFeedsOptions) (builder.Cond, error) {
	...
	// при TenantWithRoleModeEnabled = true и type == 0 получаем список action по организациям
	if setting.SourceControl.TenantWithRoleModeEnabled && opts.RequestedUser.Type == 0 {
		cond = cond.And(builder.In("user_id", opts.OrganizationIDs)).And(builder.In("repo_id", opts.AllowedRepoIDs))
	} else if opts.RequestedUser != nil {
