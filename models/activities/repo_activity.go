...
func GetActivityStatsTopAuthors(ctx context.Context, repo *repo_model.Repository, timeFrom time.Time, count int) ([]*ActivityAuthorData, int64, error) {
	gitRepo, closer, err := git.RepositoryFromContextOrOpen(ctx, repo.OwnerName, repo.Name, repo.RepoPath())
	if err != nil {
		return nil, 0, fmt.Errorf("OpenRepository: %w", err)
	}
	defer closer.Close()

	code, err := gitRepo.GetCodeActivityStats(timeFrom, "")
	if err != nil {
		return nil, 0, fmt.Errorf("FillFromGit: %w", err)
	}
	if code.Authors == nil {
		return nil, 0, nil
	}
	...
	for _, v := range code.Authors {
		...
		if u == nil {
			u, err = user_model.GetUserByName(ctx, v.Name)
		}
	...
	authorCountInAllBranches := int64(len(v))

	return v[:cnt], authorCountInAllBranches, nil
}
