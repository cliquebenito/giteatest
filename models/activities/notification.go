...
// GetUsersFromNotificationsByIssueID получаем список ids для users по issue id
func GetUsersFromNotificationsByIssueID(ctx context.Context, issueID int64) ([]int64, error) {
	var userIDs []int64
	...
	if err := db.GetEngine(ctx).
		...
		log.Error("Error has occurred while getting users from notification by issue_id: %d: v%", issueID, err)
		return nil, fmt.Errorf("getting users by issue_id: %w", err)
	}
