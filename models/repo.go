...
func DeleteRepository(doer *user_model.User, uid, repoID int64) error {
	...
	if err := db.DeleteBeans(ctx,
		...
		&protected_branch.ProtectedBranch{RepoID: repoID},
		...
		&repo_model.ScRepoLicenses{RepositoryID: repoID},
	); err != nil {
	...
	repoKeyDB := repo_model.NewRepoKeyDB(db.GetEngine(ctx))
	if err := repoKeyDB.DeleteRepoKeyByRepoID(ctx, strconv.FormatInt(repo.ID, 10)); err != nil {
		log.Error("Error has occurred while delete repokey when repo is going to be deleted: %v", err)
		return fmt.Errorf("Failed to delete repokey: %w", err)
	}

	codeHubCounterDB := internal_metric_counter_db.New(db.GetEngine(ctx))
	if err := codeHubCounterDB.DeleteCounter(ctx, repo.ID); err != nil {
		log.Error("Error has occurred while delete codehub counter when repo is going to be deleted: %v", err)
		return fmt.Errorf("failed to delete codehub counter: %w", err)
	}
