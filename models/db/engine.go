...
func newXORMEngine() (*xorm.Engine, error) {
	...
	g, err = gorm.Open(postgres.New(postgres.Config{
		...
		NamingStrategy: schema.NamingStrategy{
			TablePrefix: setting.Database.Schema,
		}})
	return engine, nil
}
...
func RoleModelAdapter() (*casbingormadapter.Adapter, error) {
	prefix := fmt.Sprintf("%s.", setting.Database.Schema)

	rawGormAdapter, err := gormadapter.NewAdapterByDBUseTableName(g, prefix, "")
	if err != nil {
		return nil, fmt.Errorf("create gorm adapter: %w", err)
	}

	return casbingormadapter.New(rawGormAdapter), nil
}
