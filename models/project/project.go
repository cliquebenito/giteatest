...
type ErrProjectWithNameNotExist struct {
	Name string
}

// IsErrProjectWithNameNotExist checks if an error is a ErrProjectWithNameNotExist
func IsErrProjectWithNameNotExist(err error) bool {
	return errors.As(err, &ErrProjectWithNameNotExist{})
}

func (err ErrProjectWithNameNotExist) Error() string {
	return fmt.Sprintf("Err: projects does not exist [name: %s]", err.Name)
}
