...
// GetSystemOrDefaultWebhookWithParams returns admin system or default webhook by given ID.
func GetSystemOrDefaultWebhookWithParams(ctx context.Context, id, repoID, ownerID int64) (*Webhook, error) {
	webhook := &Webhook{ID: id}
	has, err := db.GetEngine(ctx).
...
// DeleteDefaultSystemWebhookWithParams deletes an admin-configured default or system webhook
func DeleteDefaultSystemWebhookWithParams(ctx context.Context, id, repoID, ownerID int64) error {
	return db.WithTx(ctx, func(ctx context.Context) error {
		count, err := db.GetEngine(ctx).
			Where("repo_id=? AND owner_id=?", repoID, ownerID).
			Delete(&Webhook{ID: id})
