...
func (err ErrWebhookNotExist) Error() string {
	return fmt.Sprintf("Err: webhook does not exist [id: %d]", err.ID)
}
...
type ErrWebHookLimit struct {
	RepoID int64
}

// IsErrWebHookLimit checks if an error is a ErrWebHookLimit.
func IsErrWebHookLimit(err error) bool {
	_, ok := err.(ErrWebHookLimit)
	return ok
}

func (err ErrWebHookLimit) Error() string {
	return fmt.Sprintf("Err: webhook limit (20) reached [repo_id: %d]", err.RepoID)
}
...
// webHookLimit - количество веб-хуков, которое можно привязать к одному репозиторию
...
func (w *Webhook) HasPullRequestApprovedEvent() bool {
	return w.SendEverything ||
		(w.ChooseEvents && w.HookEvents.PullRequestReviewApproved)
}
...
func (w *Webhook) HasPullRequestRejectedEvent() bool {
	return w.SendEverything ||
		(w.ChooseEvents && w.HookEvents.PullRequestReviewRejected)
}
...
func (w *Webhook) HasPullRequestReviewCommentEvent() bool {
	return w.SendEverything ||
		(w.ChooseEvents && w.HookEvents.PullRequestReviewComment)
}

// HasPullRequestReview returns true if hook enabled pull request review event.
func (w *Webhook) HasPullRequestReview() bool {
	return w.SendEverything ||
...
func (w *Webhook) EventCheckers() []struct {
	...
	return []struct {
		...
		{w.HasPullRequestReviewCommentEvent, webhook_module.HookEventPullRequestReviewComment},
		{w.HasPullRequestSyncEvent, webhook_module.HookEventPullRequestSync},
		{w.HasPullRequestReview, webhook_module.HookEventPullRequestReview},
...
func CreateWebhook(ctx context.Context, w *Webhook) error {
	...
	count, err := db.GetEngine(ctx).Where("repo_id = ?", w.RepoID).Count(&Webhook{})
	if err != nil {
		return fmt.Errorf("failed to count webhooks: %w", err)
	}
	if count >= webHookLimit {
		return ErrWebHookLimit{RepoID: w.RepoID}
	}
