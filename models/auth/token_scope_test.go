...
func TestAccessTokenScope_Normalize(t *testing.T) {
	tests := []struct {
		...
		{"repo,admin:org,admin:public_key,admin:repo_hook,admin:org_hook,admin:user_hook,notification,user,delete_repo,package,admin:gpg_key,admin:application,admin:privileges,project,tenant,codehub", "all", nil},
		{"repo,admin:org,admin:public_key,admin:repo_hook,admin:org_hook,admin:user_hook,notification,user,delete_repo,package,admin:gpg_key,admin:application,admin:privileges,project,tenant,codehub,sudo", "all,sudo", nil},
		{"all", "all", nil},
	}
...
func TestAccessTokenScope_HasScope(t *testing.T) {
	tests := []struct {
		...
		{"admin:privileges", "admin:privileges", true, nil},
		{"admin:privileges", "write:privileges", true, nil},
		{"admin:privileges", "read:privileges", true, nil},
		{"project", "read:project", true, nil},
		{"project", "write:project", true, nil},
		{"write:project", "read:project", true, nil},
		{"tenant", "read:tenant", true, nil},
		{"tenant", "write:tenant", true, nil},
		{"write:tenant", "read:tenant", true, nil},
	}
