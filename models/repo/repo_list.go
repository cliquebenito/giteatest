...
type SearchRepoOptions struct {
	...
	// AdminPanel админская панель пользователя
	AdminPanel bool
	// OnlyShowCodeHub показывать репозитории только с отметкой codehub
	OnlyShowCodeHub bool
	// AllowedIDs массис repository_id к котороым у пользователя есть доступ
...
func SearchRepositoryCondition(opts *SearchRepoOptions) builder.Cond {
	...
	// TenantWithRoleModeEnabled = true возвращаем список репозиториев по проектам по тенантам
	if setting.SourceControl.TenantWithRoleModeEnabled && !opts.AdminPanel {
		accessCond := builder.NewCond()
		if len(opts.AllowedRepoIDs) > 0 {
			accessCond = builder.In("id", opts.AllowedRepoIDs)
		} else {
			accessCond = builder.In("owner_id", opts.OwnerIDs)
		}
	...
	if opts.OnlyShowCodeHub {
		codeHubMark := marks.GetCodeHubMark(setting.CodeHub.CodeHubMarkLabelName)
		cond = cond.And(
			builder.In("id", builder.Select("repo_id").From("`repo_marks`").Where(
				builder.Eq{"mark_key": codeHubMark.Key()},
			)))
	}
...
func GetUserRepositories(opts *SearchRepoOptions) (RepositoryList, int64, error) {
	...
	if opts.LowerNames != nil && len(opts.LowerNames) > 0 && !setting.SourceControl.TenantWithRoleModeEnabled {
