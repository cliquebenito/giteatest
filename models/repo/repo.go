...
type Repository struct {
	...
	RepoMarks       []*Mark                                          `xorm:"-"`
	ExternalMetric  *external_metric_counter.ExternalMetricCounter   `xorm:"-"`
	InternalMetrics []*internal_metric_counter.InternalMetricCounter `xorm:"-"`
...
// LinkWithoutSub returns the repository relative url without sub
func (repo *Repository) LinkWithoutSub() string {
	return "/" + url.PathEscape(repo.OwnerName) + "/" + url.PathEscape(repo.Name)
}
...
func ComposeHTTPSCloneURL(owner, repo string) string {
	link := setting.AppURL
	if isOneWork() {
		link = setting.IAM.BaseURL
	}

	return fmt.Sprintf("%s%s/%s.git", link, url.PathEscape(owner), url.PathEscape(repo))
}
...
func ComposeSSHCloneURL(ownerName, repoName string) string {
	...
	if isOneWork() {
		sshDomain = setting.IAM.SSHDomain
	}

	if setting.SSH.Port != 22 {
		return fmt.Sprintf("ssh://%s@%s/%s/%s.git", sshUser,
			net.JoinHostPort(sshDomain, strconv.Itoa(setting.SSH.Port)),
	...
	ip := net.ParseIP(sshDomain)
	if ip != nil && ip.To4() == nil {
		sshDomain = fmt.Sprintf("[%s]", sshDomain)
	}
...
func isOneWork() bool {
	return setting.IAM.Enabled && setting.OneWork.Enabled
}
...
// IsErrRepositoryNotExist checks if an error is a ErrRepositoryNotExist.
func IsErrRepositoryNotExist(err error) bool {
	return errors.As(err, &ErrRepositoryNotExist{})
}

func (err ErrRepositoryNotExist) Error() string {
	return fmt.Sprintf("Err: repository does not exist [name: %s]",
		err.Name)
}

// Unwrap unwraps this error as a ErrRepositoryNotExist error
func (err ErrRepositoryNotExist) Unwrap() error {
	return util.ErrNotExist
}
