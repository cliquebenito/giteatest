...
func InsertEncryptedSecret(ctx context.Context, ownerID, repoID int64, name, data string) (*Secret, error) {
	...
	if err != nil {
		log.Error("Error has occurred while encrypting a secret: %v", err)
		return nil, fmt.Errorf("encryption: %w", err)
	}
	secret := newSecret(ownerID, repoID, name, encrypted)
	if err := secret.Validate(); err != nil {
		log.Error("Error has occurred while validating a secret: %v", err)
		return secret, fmt.Errorf("validation: %w", err)
	}
