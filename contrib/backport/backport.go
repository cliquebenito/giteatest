...
func main() {
	...
	app.Description = `Backport will look-up the PR in SourceControl's git log and attempt to cherry-pick it on the current version`
	...
	app.Flags = []cli.Flag{
		...
		cli.StringFlag{
			Name:  "upstream",
			Value: "origin",
			Usage: "Upstream remote for the SourceControl upstream",
		},
		cli.StringFlag{
			Name:  "release-branch",
			Value: "",
		...
		cli.StringFlag{
			Name:  "remote",
			Value: "",
			Usage: "Remote for your fork of the SourceControl upstream",
		},
		cli.StringFlag{
			Name:  "fork-user",
			Value: "",
