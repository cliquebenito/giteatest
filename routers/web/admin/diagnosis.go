...
func MonitorDiagnosis(ctx *context.Context) {
	...
	if err != nil {
		...
		audit.CreateAndSendEvent(audit.DiagnosisReportDownload, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	_ = pprof.Lookup("goroutine").WriteTo(f, 1)
	audit.CreateAndSendEvent(audit.DiagnosisReportDownload, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, nil)
}
