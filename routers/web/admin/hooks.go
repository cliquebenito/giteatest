...
func DeleteDefaultOrSystemWebhook(ctx *context.Context) {
	...
	if err := webhook.DeleteDefaultSystemWebhook(ctx, ctx.FormInt64("id")); err != nil {
		...
		auditParams["error"] = "Error has occurred while deleting default or system hook"
		audit.CreateAndSendEvent(audit.DefaultOrSystemHookRemoveEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
	} else {
		ctx.Flash.Success(ctx.Tr("repo.settings.webhook_deletion_success"))
		audit.CreateAndSendEvent(audit.DefaultOrSystemHookRemoveEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, auditParams)
	}
