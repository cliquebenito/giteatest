...
func Authentications(ctx *context.Context) {
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Error has occurred while getting authentication sources",
		}
		audit.CreateAndSendEvent(audit.AuthenticationSourceManagementOpen, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}

	ctx.Data["Total"] = auth.CountSources()
	audit.CreateAndSendEvent(audit.AuthenticationSourceManagementOpen, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, nil)
...
func NewAuthSourcePost(ctx *context.Context) {
	...
	auditParams := map[string]string{
		"auth_type":            auth.Type(form.Type).String(),
		"auth_name":            form.Name,
		"auth_is_active":       strconv.FormatBool(form.IsActive),
		"auth_is_sync_enabled": strconv.FormatBool(form.IsSyncEnabled),
	}
	...
	case auth.OAuth2:
		...
		if oauth2Config.Provider == "openidConnect" {
			...
			if err != nil || (discoveryURL.Scheme != "http" && discoveryURL.Scheme != "https") {
				...
				auditParams["error"] = "Invalid auto discovery URL"
				audit.CreateAndSendEvent(audit.AuthenticationSourceAdd, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
				return
			}
		}
	case auth.SSPI:
		...
		if err != nil {
			...
			auditParams["error"] = "Error has occurred while parsing SSPI config"
			audit.CreateAndSendEvent(audit.AuthenticationSourceAdd, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
		...
		if err != nil || len(existing) > 0 {
			...
			auditParams["error"] = "An authentication source of this type already exists"
			audit.CreateAndSendEvent(audit.AuthenticationSourceAdd, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	default:
		ctx.Error(http.StatusBadRequest)
		auditParams["error"] = "Unknown authentication type"
		audit.CreateAndSendEvent(audit.AuthenticationSourceAdd, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err := auth.CreateSource(&auth.Source{
		...
		if auth.IsErrSourceAlreadyExist(err) {
			...
			auditParams["error"] = "The authentication source already exists"
			audit.CreateAndSendEvent(audit.AuthenticationSourceAdd, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			...
			auditParams["error"] = "Unable to initialize OpenID connect provider"
			audit.CreateAndSendEvent(audit.AuthenticationSourceAdd, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		} else {
			ctx.ServerError("auth.CreateSource", err)
			auditParams["error"] = "Error has occurred while creating source"
			audit.CreateAndSendEvent(audit.AuthenticationSourceAdd, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		}
		return
	}
	...
	audit.CreateAndSendEvent(audit.AuthenticationSourceAdd, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, auditParams)
...
func EditAuthSourcePost(ctx *context.Context) {
	...
	auditParams := map[string]string{
		"auth_id":              strconv.FormatInt(ctx.ParamsInt64(":authid"), 10),
		"auth_type":            auth.Type(form.Type).String(),
		"auth_name":            form.Name,
		"auth_is_active":       strconv.FormatBool(form.IsActive),
		"auth_is_sync_enabled": strconv.FormatBool(form.IsSyncEnabled),
	...
	if err != nil {
		ctx.ServerError("auth.GetSourceByID", err)
		auditParams["error"] = "Error has occurred while getting source by id"
		audit.CreateAndSendEvent(audit.AuthenticationSourceEdit, auditValues.DoerName, auditValues.DoerID, audit.StatusFailure, auditValues.RemoteAddress, auditParams)
		return
	}
	ctx.Data["Source"] = source
	ctx.Data["HasTLS"] = source.HasTLS()
	...
	old, err := auth.GetSourceByID(source.ID)
	if err != nil {
		log.Error("Error has occurred while retrieving previous source information for source: %s, error: %v", source.Name, err)
		ctx.Error(http.StatusBadRequest, fmt.Sprintf("Аутентификации с именем %s не существует", source.Name))
		auditParams["error"] = "Error has occurred while retrieving previous source information"
		audit.CreateAndSendEvent(audit.AuthenticationSourceEdit, auditValues.DoerName, auditValues.DoerID, audit.StatusFailure, auditValues.RemoteAddress, auditParams)
		return
	}
	oldValueBytes, jsonMarshalErr := json.Marshal(old)
	if jsonMarshalErr != nil {
		log.Error("Error has occurred while converting data to bytes: %v, error: %v", old, jsonMarshalErr)
	...
	err = auth.UpdateSource(source)

	newValueBytes, jsonMarshalErr := json.Marshal(source)
	if jsonMarshalErr != nil {
		log.Error("Error has occurred while converting data to bytes: %v, error: %v", source, jsonMarshalErr)
		ctx.Error(http.StatusBadRequest, "Не удалось преобразовать данные")
		auditParams["error"] = "Error has occurred while converting data to bytes"
		audit.CreateAndSendEvent(audit.AuthenticationSourceEdit, auditValues.DoerName, auditValues.DoerID, audit.StatusFailure, auditValues.RemoteAddress, auditParams)
	}
	...
	if err != nil {
		if auth.IsErrSourceAlreadyExist(err) {
			...
			auditParams["error"] = "Error has occurred while updating source"
			audit.CreateAndSendEvent(audit.AuthenticationSourceEdit, auditValues.DoerName, auditValues.DoerID, audit.StatusFailure, auditValues.RemoteAddress, auditParams)
		}
		return
	}
	log.Trace("Authentication changed by admin(%s): %d", auditValues.DoerName, source.ID)

	audit.CreateAndSendEvent(audit.AuthenticationSourceEdit, auditValues.DoerName, auditValues.DoerID, audit.StatusSuccess, auditValues.RemoteAddress, auditParams)
...
func DeleteAuthSource(ctx *context.Context) {
	...
	if err != nil {
		ctx.ServerError("auth.GetSourceByID", err)
		auditParams["error"] = "Error has occurred while getting source by id"
		audit.CreateAndSendEvent(audit.AuthenticationSourceDelete, auditValues.DoerName, auditValues.DoerID, audit.StatusFailure, auditValues.RemoteAddress, auditParams)
		return
	}
	...
	if err = auth_service.DeleteSource(source); err != nil {
		if auth.IsErrSourceInUse(err) {
			...
			auditParams["error"] = "The authentication source is still in use"
		} else {
			ctx.Flash.Error(fmt.Sprintf("auth_service.DeleteSource: %v", err))
			auditParams["error"] = "Error has occurred while deleting source"
		}
		audit.CreateAndSendEvent(audit.AuthenticationSourceDelete, auditValues.DoerName, auditValues.DoerID, audit.StatusFailure, auditValues.RemoteAddress, auditParams)
	...
	log.Trace("Authentication deleted by admin(%s): %d", auditValues.DoerName, source.ID)

	audit.CreateAndSendEvent(audit.AuthenticationSourceDelete, auditValues.DoerName, auditValues.DoerID, audit.StatusSuccess, auditValues.RemoteAddress, auditParams)
