...
func Config(ctx *context.Context) {
	...
	if err != nil {
		auditParams := map[string]string{
			"error": "Error has occurred while getting all settings",
		}
		audit.CreateAndSendEvent(audit.AdminConfigurationOpen, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
	...
	if len(os.Getenv("SC_WORK_DIR")) > 0 {
		envVars["SC_WORK_DIR"] = &envVar{"SC_WORK_DIR", os.Getenv("SC_WORK_DIR")}
	}
	if len(os.Getenv("SC_CUSTOM")) > 0 {
		envVars["SC_CUSTOM"] = &envVar{"SC_CUSTOM", os.Getenv("SC_CUSTOM")}
	}
	...
	audit.CreateAndSendEvent(audit.AdminConfigurationOpen, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, nil)
...
func ChangeConfig(ctx *context.Context) {
	...
	if key == "" {
		...
		auditParams["error"] = "Setting key is empty"
		audit.CreateAndSendEvent(audit.AdminConfigurationChange, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, nil)
		return
	}
	value := ctx.FormString("value")
	version := ctx.FormInt("version")

	auditParams["new_value"] = value
	auditParams["version"] = strconv.Itoa(version)
	...
	if check, ok := changeConfigChecks[key]; ok {
		if err := check(ctx, value); err != nil {
			...
			auditParams["error"] = "Set setting failed"
			audit.CreateAndSendEvent(audit.AdminConfigurationChange, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, nil)
			return
		}
	}
	...
	audit.CreateAndSendEvent(audit.AdminConfigurationChange, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, auditParams)
