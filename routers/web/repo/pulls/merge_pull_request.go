...
func (s Server) MergePullRequest(ctx *context.Context) {
	...
	if s.taskTrackerEnabled {
		...
		if err := s.updatePullRequestStatusWhileMerge(ctx, issue, userName); err != nil {
			log.Debug("run updating the issue's status: %v", err)
			auditParams["error"] = "Error has occurred trying to do MR from unit"
