...
func handleMigrateError(ctx *context.Context, owner *user_model.User, err error, name string, tpl base.TplName, form *forms.MigrateRepoForm) {
	auditParams := map[string]string{
		"migration_url": form.CloneAddr,
		"repository":    form.RepoName,
	}
	if setting.Repository.DisableMigrations {
		ctx.Error(http.StatusForbidden, "MigrateError: the site administrator has disabled migrations")
		auditParams["error"] = "The site administrator has disabled migrations"
		audit.CreateAndSendEvent(audit.RepositoryImportEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}

	switch {
	case migrations.IsRateLimitError(err):
		auditParams["error"] = "Visit rate limit"
		ctx.RenderWithErr(ctx.Tr("form.visit_rate_limit"), tpl, form)
	case migrations.IsTwoFactorAuthError(err):
		auditParams["error"] = "Two factor authentication required"
	...
	default:
		...
		if strings.Contains(err.Error(), "Authentication failed") ||
			...
			auditParams["error"] = "Authentication failed"
			ctx.RenderWithErr(ctx.Tr("form.auth_failed", err.Error()), tpl, form)
		} else if strings.Contains(err.Error(), "fatal:") {
			ctx.Data["Err_CloneAddr"] = true
			auditParams["error"] = "Error has occurred while migrating repository"
...
func handleMigrateRemoteAddrError(ctx *context.Context, err error, tpl base.TplName, form *forms.MigrateRepoForm) {
	...
	if models.IsErrInvalidCloneAddr(err) {
		...
		case addrErr.IsInvalidPath:
			auditParams["error"] = "Invalid path"
...
func MigratePost(ctx *context.Context) {
	...
	if form.Mirror && setting.Mirror.DisableNewPull {
		...
		auditParams["error"] = "The site administrator has disabled creation of new mirrors"
		audit.CreateAndSendEvent(audit.RepositoryImportEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if form.LFS && len(form.LFSEndpoint) > 0 {
		...
		if ep == nil {
			...
			auditParams["error"] = "Invalid LFS endpoint"
			audit.CreateAndSendEvent(audit.RepositoryImportEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
	...
	if err == nil {
		audit.CreateAndSendEvent(audit.RepositoryImportEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, auditParams)
