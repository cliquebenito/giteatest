...
func RefBlame(ctx *context.Context) {
	...
	if ctx.Repo == nil || ctx.Repo.Repository == nil {
		log.Error("Error has occurred while getting organization ID: no repository was found in context.")
		ctx.NotFound("Organization ID was not found", nil)
		return
	}
	...
	repo, err := git.OpenRepository(ctx, userName, repoName, ctx.Repo.Repository.RepoPath())
	if err != nil {
		log.Error("Error has occurred while opening a repository: %v", err)
		return
	}

	ctxWithCancel, cancel := contextDefault.WithCancel(repo.Ctx)
	defer cancel()
	rawBlameStream, err := repo.CommitClient.RawBlame(ctxWithCancel, &gitalypb.RawBlameRequest{
	...
	blameBuffer := strings.Builder{}
	nextPart, err := rawBlameStream.Recv()
	for err != io.EOF {
		if err != nil {
			log.Error("Error has occurred while receiving blame part: %v", err)
			return
		}

		piece := nextPart.GetData()
		blameBuffer.Write(piece)
		nextPart, err = rawBlameStream.Recv()
	}

	blameBufferReader := bufio.NewReader(strings.NewReader(blameBuffer.String()))
	blameReader := git.NewBlameReaderFromBuffer(blameBufferReader)
