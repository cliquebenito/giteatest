...
func Activity(ctx *context.Context) {
	...
	//allowedRoleModel, errCheckPermission := role_model.CheckPermissionByRoleModelOrCustomPrivileges(ctx, role_model.ConfCustomPrivilege{
	//	User:            ctx.Doer,
	//	TenantID:        ctx.Data["TenantID"].(string),
	//	Org:             &organization.Organization{ID: ctx.Repo.Repository.OwnerID},
	//	Action:          action,
	//	RepoID:          ctx.Repo.Repository.ID,
	//	CustomPrivilege: role_model.ViewBranch,
	//})
	//if errCheckPermission != nil {
	//	log.Error("Error has occurred while checking user permissions: %v", errCheckPermission)
	//	ctx.ServerError("Error has occurred while checking user permissions: %v", errCheckPermission)
	//	return
	//}
...
func ActivityAuthors(ctx *context.Context) {
	...
	var err error
	authors, _, err := activities_model.GetActivityStatsTopAuthors(ctx, ctx.Repo.Repository, timeFrom, 10)
	if err != nil {
		ctx.ServerError("GetActivityStatsTopAuthors", err)
		return
	}
