...
func renderFile(ctx *context.Context, entry *git.TreeEntry, treeLink, rawLink string) {
	...
	if ctx.Repo.TreePath == ".editorconfig" {
		...
		// отвечает за отображение ворнингов при переходе в файл codeowners
	} else if util.SliceContains([]string{"CODEOWNERS", "docs/CODEOWNERS"}, ctx.Repo.TreePath) {
		if data, err := blob.GetBlobContent(); err == nil {
			_, _, warnings := issue_model.GetCodeOwnersFromContent(ctx, data)
			if len(warnings) > 0 {
...
func checkHomeCodeViewable(ctx *context.Context) {
	...
	if len(ctx.Repo.Units) > 0 {
		...
		ctx.Data["Org"] = organization.Organization{ID: ctx.Repo.Repository.OwnerID}
		...
		if err != nil {
			log.Error(fmt.Sprintf("Error has occurred while getting sonar settings for repository_id %v: %v", ctx.Repo.Repository.ID, err))
			ctx.ServerError("Failed getting sonar settings: %v", err)
			return
		}
	...
	ctx.NotFound("HomeOrg", fmt.Errorf(ctx.Tr("units.error.no_unit_allowed_repo")))
}
...
func (r RepoServer) Home(ctx *context.Context) {
	fmt.Println("TraceID")
	fmt.Println(ctx.Data["TraceID"])
	...
	if ctx.Written() {
		auditParams["error"] = "Error has occurred while checking home code viewable"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
...
func (r RepoServer) renderCode(ctx *context.Context) {
	logTracer := trace.NewLogTracer()
	message := logTracer.CreateTraceMessage(ctx)
	err := logTracer.Trace(message)
	if err != nil {
	...
	if ctx.Repo.Commit == nil || ctx.Repo.Repository.IsEmpty || ctx.Repo.Repository.IsBroken() {
		...
		if showEmpty {
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusSuccess, ctx.Req.RemoteAddr, auditParams)
		...
		if err = repo_module.UpdateRepoSize(ctx, ctx.Repo.Repository); err != nil {
			...
			auditParams["error"] = "Error has occurred while updating repository size"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	...
	allowed, err := role_model.CheckUserPermissionToOrganization(ctx, &user_model.User{ID: ctx.Doer.ID},
		...
		action)
	if err != nil {
		log.Error("User %s does not have permission to access the repository %s", ctx.Doer.Name, ctx.Repo.Repository.FullName())
		ctx.Error(http.StatusForbidden, "User does not have permission to access the repository")
		return
	}
	if !allowed {
		allow, err := role_model.CheckUserPermissionToTeam(ctx,
	...
	if ctx.Written() {
		auditParams["error"] = "Error has occurred while rendering repository topics"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		ctx.NotFoundOrServerError("Repo.Commit.GetTreeEntryByPath", git.IsErrNotExist, err)
		auditParams["error"] = "Error has occurred while getting tree entry by path"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if ctx.Written() {
		auditParams["error"] = "Error has occurred while checking citation file"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if ctx.Written() {
		auditParams["error"] = "Error has occurred while rendering language stats"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	if setting.SourceControl.Enabled {
		renderGitLicenseInfo(ctx)
		if ctx.Written() {
			auditParams["error"] = "Error has occurred while rendering licenses info"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	}
	...
	if ctx.Written() {
		auditParams["error"] = "Error has occurred while rendering"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if r.marksEnabled {
		...
		for _, mark := range r.processedMarks {
			marksDef[mark.Key()] = mark.Label()
		}
		marks, err := r.GetRepoMarks(ctx, ctx.Repo.Repository.ID)
		if err != nil {
		...
		ctx.Repo.Repository.RepoMarks = marksResult[ctx.Repo.Repository.ID]
	}
...
func renderGitLicenseInfo(ctx *context.Context) {
	repo, err := repo_model.GetRepositoryByID(ctx, ctx.Repo.Repository.ID)
	if err != nil {
		log.Error("renderGitLicenseInfo repo_model.GetRepositoryByID failed: %v", err)
		ctx.Error(http.StatusNotFound, fmt.Sprintf("renderGitLicenseInfo repo_model.GetRepositoryByID failed repository_id %v not found : %v", ctx.Repo.Repository.ID, err))
		return
	}
	gitRepo, err := git.OpenRepository(ctx, repo.OwnerName, repo.Name, repo.RepoPath())
	if err != nil {
		log.Error("renderGitLicenseInfo git.OpenRepository failed: %v", err)
		ctx.Error(http.StatusNotFound, fmt.Sprintf("renderGitLicenseInfo git.OpenRepository failed repo_path %s not found: %v", repo.RepoPath(), err))
		return
	}
	defer gitRepo.Close()
	infoLicenses, err := repo_model.GetLicenseInfoByCommitIDAndBranch(ctx, repo.ID, "", repo.DefaultBranch)
	if err != nil {
		log.Error("renderGitLicenseInfo repo_model.GetLicenseInfoByCommitIDAndBranch failed while getting licences for repository_id %v: %v", repo.ID, err)
		ctx.Error(http.StatusNotFound, fmt.Sprintf("renderGitLicenseInfo repo_model.GetLicenseInfoByCommitIDAndBranch there is not license for repository_id %v and branch %s failed: %v", repo.ID, repo.DefaultBranch, err))
		return
	}
	if len(infoLicenses) == 0 {
		return
	}
	// fileNamePath структура с именем лицензии и пути к файлу
	type fileNamePath struct {
		Name string
		Path string
	}
	licenseNames := make([]fileNamePath, len(infoLicenses))
	for idx, infoLicense := range infoLicenses {
		urlPath := strings.Split(infoLicense.PathFile, "/")
		fileLicense := fileNamePath{
			Name: infoLicense.NameLicense,
			Path: fmt.Sprintf("%s%s/%s/src/%s/%s", setting.AppURL, urlPath[0], urlPath[1], infoLicense.BranchName, urlPath[2]),
		}
		licenseNames[idx] = fileLicense
	}
	ctx.Data["Licenses"] = licenseNames
}
