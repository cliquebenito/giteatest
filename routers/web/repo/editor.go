...
func DiffPreviewPost(ctx *context.Context) {
	...
	diff, err := files_service.GetDiffPreview(ctx, ctx.Repo, ctx.Repo.BranchName, treePath, form.Content, entry)
	if err != nil {
...
func UploadFilePost(ctx *context.Context) {
	...
	if len(form.Files) == 0 {
		ctx.RenderWithErr(ctx.Tr("repo.editor.no_files_to_upload"), tplUploadFile, &form)
		return
	}
