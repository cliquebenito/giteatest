...
func (r RepoServer) getBlobForEntry(ctx *context.Context) (blob *git.Blob, lastModified time.Time) {
...
func (r RepoServer) SingleDownload(ctx *context.Context) {
	blob, lastModified := r.getBlobForEntry(ctx)
	if blob == nil {
		return
	}
...
func (r RepoServer) SingleDownloadOrLFS(ctx *context.Context) {
	blob, lastModified := r.getBlobForEntry(ctx)
	if blob == nil {
		return
	}
