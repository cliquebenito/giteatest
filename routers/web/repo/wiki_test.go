...
func TestWiki(t *testing.T) {
	...
	ctx.SetParams("*", "HomeOrg")
	test.LoadRepo(t, ctx, 1)
	Wiki(ctx)
	assert.EqualValues(t, http.StatusOK, ctx.Resp.Status())
	assert.EqualValues(t, "HomeOrg", ctx.Data["Title"])
	assertPagesMetas(t, []string{"HomeOrg", "Page With Image", "Page With Spaced Name", "Unescaped File"}, ctx.Data["Pages"])
}
...
func TestEditWiki(t *testing.T) {
	...
	ctx := test.MockContext(t, "user2/repo1/wiki/HomeOrg?action=_edit")
	ctx.SetParams("*", "HomeOrg")
	test.LoadUser(t, ctx, 2)
	test.LoadRepo(t, ctx, 1)
	EditWiki(ctx)
	assert.EqualValues(t, http.StatusOK, ctx.Resp.Status())
	assert.EqualValues(t, "HomeOrg", ctx.Data["Title"])
	assert.Equal(t, wikiContent(t, ctx.Repo.Repository, "HomeOrg"), ctx.Data["content"])
}
...
func TestEditWikiPost(t *testing.T) {
	for _, title := range []string{
		...
		unittest.PrepareTestEnv(t)
		ctx := test.MockContext(t, "user2/repo1/wiki/HomeOrg?action=_new")
		ctx.SetParams("*", "HomeOrg")
		test.LoadUser(t, ctx, 2)
		test.LoadRepo(t, ctx, 1)
		web.SetForm(ctx, &forms.NewWikiForm{
		...
		assert.Equal(t, wikiContent(t, ctx.Repo.Repository, wiki_service.UserTitleToWebPath("", title)), content)
		if title != "HomeOrg" {
			assertWikiNotExists(t, ctx.Repo.Repository, "HomeOrg")
		}
	}
}
...
func TestDeleteWikiPagePost(t *testing.T) {
	...
	ctx := test.MockContext(t, "user2/repo1/wiki/HomeOrg?action=_delete")
	test.LoadUser(t, ctx, 2)
	test.LoadRepo(t, ctx, 1)
