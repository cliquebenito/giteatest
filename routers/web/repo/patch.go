...
func NewDiffPatchPost(ctx *context.Context) {
	...
	defer cancel()
	applyPatchClient, err := ctx.Repo.GitRepo.OperationClient.UserApplyPatch(ctxWithCancel)
	if err != nil {
		ctx.RenderWithErr(ctx.Tr("repo.editor.fail_to_apply_patch", err), tplPatchFile, &form)
		return
	}
	err = applyPatchClient.Send(&gitalypb.UserApplyPatchRequest{
	...
	err = applyPatchClient.Send(&gitalypb.UserApplyPatchRequest{
		UserApplyPatchRequestPayload: &gitalypb.UserApplyPatchRequest_Patches{
			Patches: []byte(strings.ReplaceAll(form.Content, "\r", "")),
		},
	})
	if err != nil {
		ctx.RenderWithErr(ctx.Tr("repo.editor.fail_to_apply_patch", err), tplPatchFile, &form)
	...
	applyPatchResponse, err := applyPatchClient.CloseAndRecv()
	if err != nil {
		ctx.RenderWithErr(ctx.Tr("repo.editor.fail_to_apply_patch", err), tplPatchFile, &form)
		return
	}
	...
	if form.CommitChoice == frmCommitChoiceNewBranch && ctx.Repo.Repository.UnitEnabled(ctx, unit.TypePullRequests) {
		...
		ctx.Redirect(ctx.Repo.RepoLink + "/commit/" + applyPatchResponse.BranchUpdate.CommitId)
	}
}
