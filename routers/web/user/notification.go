...
func getNotifications(ctx *context.Context) {
	...
	uniqueRepositories, err := repo_model.GetRepositoriesMapByIDs(repoIDs)
	if err != nil {
		log.Error("Error has occurred while getting repositories by repo ids: %v", err)
		ctx.ServerError("Error has occurred while getting repositories by repo ids: %v", err)
		return
	}
	...
	allowRepoIDs := make(map[int64]struct{})
	for _, repository := range uniqueRepositories {
		action := role_model.READ
		if repository.IsPrivate {
			action = role_model.READ_PRIVATE
		}

		allowRoleModel, err := role_model.CheckUserPermissionToOrganization(ctx, &user_model.User{ID: ctx.Doer.ID}, tenantID, &organization.Organization{ID: repository.OwnerID}, action)
		if err != nil {
		...
		if !allowRoleModel {
			allow, err := role_model.CheckUserPermissionToTeam(ctx, &user_model.User{ID: ctx.Doer.ID}, tenantID, &organization.Organization{ID: repository.OwnerID}, &repo_model.Repository{ID: repository.ID}, role_model.ViewBranch.String())
			if err != nil {
				log.Error("Error has occurred while checking user's permissions: %v", err)
		...
		allowRepoIDs[repository.ID] = struct{}{}
	}
...
func NotificationSubscriptions(ctx *context.Context) {
	...
	// TenantWithRoleModeEnabled = true получаем репозитории по проектам из тенантов
	if setting.SourceControl.TenantWithRoleModeEnabled {
		userPrivileges, err := utils.GetTenantsPrivilegesByUserID(ctx, ctx.Doer.ID)
		if err != nil {
			ctx.Error(http.StatusNotFound, fmt.Sprintf("utils.GetUsersPrivilegesByUserID failed: %v", err))
			return
		}

		tenantID, err := role_model.GetUserTenantId(ctx, ctx.Doer.ID)
		if err != nil {
			log.Error("could not find tenant for user %v. err: %v", ctx.Doer.ID, err)
			ctx.Error(http.StatusNotFound, fmt.Sprintf("User with id %v does not belong to any tenant. err: %v", ctx.Doer.ID, err))
		}
		...
		organizationsPrivileges := utils.ConvertPrivilegesTenantFromOrganizationsOrUsers(userPrivileges, user_model.UserTypeOrganization)
		repositoriesOrganization := make([]*repo_model.Repository, 0)
		for organizationPrivilege := range organizationsPrivileges {
			repositories, err := organization.GetOrgRepositories(ctx, organizationPrivilege)
			if err != nil {
				ctx.Error(http.StatusNotFound, fmt.Sprintf("organization.GetOrgRepositories failed: %v", err))
			}
			repositoriesOrganization = append(repositoriesOrganization, repositories...)
		}
		repositoryIDs = make([]int64, len(repositoriesOrganization))
		for idx, repository := range repositoriesOrganization {
			if repository.IsPrivate {
				allow, err := role_model.CheckUserPermissionToTeam(ctx, ctx.Doer, tenantID, &organization.Organization{ID: repository.OwnerID}, &repo_model.Repository{ID: repository.ID}, role_model.ViewBranch.String())
				if err != nil {
			...
			repositoryIDs[idx] = repository.ID
		}
	}
...
func NotificationWatching(ctx *context.Context) {
	...
	var reposAllowed repo_model.RepositoryList

	var organizationIDs []int64
	// TenantWithRoleModeEnabled = true получаем проекты по тенантам
	if setting.SourceControl.TenantWithRoleModeEnabled {
		...
		for _, v := range usersPrivileges {
			if v.User.ID == ctx.Doer.ID {
				userPrivileges = append(userPrivileges, v)
			}
		}
		repositoriesOrganization := utils.ConvertPrivilegesTenantFromOrganizationsOrUsers(userPrivileges, user_model.UserTypeOrganization)
		organizationIDs = utils.ConvertMapUserOrOrganizationsInSlice(repositoriesOrganization)
		...
		reposAllowedMap := make(map[int64]*repo_model.Repository)

		for _, vv := range organizationIDs {
			for _, v := range repos {
				if _, exists := reposAllowedMap[v.ID]; exists {
					continue
				}
				action := role_model.READ
				if v.IsPrivate {
				...
				allowed, err := role_model.CheckUserPermissionToOrganization(ctx, ctx.Doer, tenantID, &organization.Organization{ID: vv}, action)
				if err != nil {
					log.Error("Error has occurred while checking user permission to organization: %v", err)
					return
				}
				...
				if !allowed {
					allow, err := role_model.CheckUserPermissionToTeam(ctx, ctx.Doer, tenantID, &organization.Organization{ID: vv}, &repo_model.Repository{ID: v.ID}, role_model.ViewBranch.String())
					if err != nil {
				...
				reposAllowedMap[v.ID] = v
			}
		}

		for _, v := range reposAllowedMap {
			reposAllowed = append(reposAllowed, v)
		}

		total := len(reposAllowed)
		ctx.Data["Total"] = total
		ctx.Data["Repos"] = reposAllowed
