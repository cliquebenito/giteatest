...
func SignInOpenIDPost(ctx *context.Context) {
	...
	if ctx.HasError() {
		...
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Error occurs normalize openID",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Unable to find OpenID provider",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
...
func signInOpenIDVerify(ctx *context.Context) {
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Error has occurred while verifying full URL",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		if !user_model.IsErrUserNotExist(err) {
			...
			auditParams := map[string]string{
				"error": "Error has occurred while getting user by openID",
			}
			audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Error has occurred while parsing full URL",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Error has occurred while parsing query from full URL",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if u == nil && nickname != "" {
		...
		if err != nil {
			if !user_model.IsErrUserNotExist(err) {
				...
				auditParams := map[string]string{
					"error": "User with that name not exist",
					"email": email,
				}
				audit.CreateAndSendEvent(audit.UserLoginEvent, nickname, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
				return
			}
		}
		if u != nil {
...
func ConnectOpenIDPost(ctx *context.Context) {
	...
	if oid == "" {
		auditParams := map[string]string{
			"error": "Error has occurred while getting openID verified URI",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, form.UserName, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		ctx.Redirect(setting.AppSubURL + "/user/login/openid")
		return
	}
	ctx.Data["Title"] = "OpenID connect"
	...
	if err != nil {
		...
		if user_model.IsErrUserNotExist(err) {
			...
			auditParams["error"] = "Username or password incorrect"
		} else {
	...
	if err = user_model.AddUserOpenID(ctx, userOID); err != nil {
		if user_model.IsErrOpenIDAlreadyUsed(err) {
			...
			auditParams["error"] = "OpenID already used"
		...
		auditParams["error"] = "Error has occurred while adding user openID"
		audit.CreateAndSendEvent(audit.UserLoginEvent, u.Name, strconv.FormatInt(u.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
...
func RegisterOpenIDPost(ctx *context.Context) {
	...
	if oid == "" {
		ctx.Redirect(setting.AppSubURL + "/user/login/openid")
		auditParams["error"] = "Empty OpenID"
	...
	if setting.Service.AllowOnlyInternalRegistration {
		...
		auditParams["error"] = "Allow only internal registration"
		audit.CreateAndSendEvent(audit.UserCreateEvent, form.UserName, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if setting.Service.EnableCaptcha {
		if err := ctx.Req.ParseForm(); err != nil {
			ctx.ServerError("", err)
			auditParams["error"] = "Error has occurred while parsing form"
			audit.CreateAndSendEvent(audit.UserCreateEvent, form.UserName, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while trying to generate password"
		audit.CreateAndSendEvent(audit.UserCreateEvent, form.UserName, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err = user_model.AddUserOpenID(ctx, userOID); err != nil {
		if user_model.IsErrOpenIDAlreadyUsed(err) {
			...
			auditParams["error"] = "OpenID already used"
			audit.CreateAndSendEvent(audit.UserCreateEvent, u.Name, strconv.FormatInt(u.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
		ctx.ServerError("AddUserOpenID", err)
		auditParams["error"] = "Error has occurred while trying to add user OpenID"
		audit.CreateAndSendEvent(audit.UserCreateEvent, u.Name, strconv.FormatInt(u.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
