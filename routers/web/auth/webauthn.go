...
func WebAuthnLoginAssertion(ctx *context.Context) {
	...
	if !ok || idSess == 0 {
		...
		auditParams := map[string]string{
			"error": "Error has occurred while getting two factor uid",
		}
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Error has occurred while getting user by id",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		ctx.ServerError("UserSignIn", err)
		auditParams["error"] = "Error has occurred while checking exists web authn credentials for uid"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	if !exists {
		ctx.ServerError("UserSignIn", errors.New("no device registered"))
		auditParams["error"] = "Web authn credentials for uid not exist"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while beginning login"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err := ctx.Session.Set("webauthnAssertion", sessionData); err != nil {
		...
		auditParams["error"] = "Error has occurred while setting web authn assertion to session"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
...
func WebAuthnLoginAssertionPost(ctx *context.Context) {
	...
	if !ok || !okData || sessionData == nil || idSess == 0 {
		ctx.ServerError("UserSignIn", errors.New("not in WebAuthn session"))
		auditParams := map[string]string{
			"error": "Error has occurred while getting web authn assertion",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while parsing credential request response"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		...
		log.Info("Failed authentication attempt for %s from %s: %v", user.Name, ctx.RemoteAddr(), err)
		ctx.Status(http.StatusForbidden)
		auditParams["error"] = "Error has occurred while validating login"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if cred.Authenticator.CloneWarning {
		...
		ctx.Status(http.StatusForbidden)
		auditParams["error"] = "Cloned credential"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while getting web authn credential by credID"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err := dbCred.UpdateSignCount(); err != nil {
		...
		auditParams["error"] = "Error has occurred while updating sign count"
		audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if ctx.Session.Get("linkAccount") != nil {
		if err := externalaccount.LinkAccountFromStore(ctx.Session, user); err != nil {
			...
			auditParams["error"] = "Error has occurred while linking account from store"
			audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	}
	...
	audit.CreateAndSendEvent(audit.UserLoginEvent, user.Name, strconv.FormatInt(user.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, auditParams)
