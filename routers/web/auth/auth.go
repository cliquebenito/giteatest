...
func SignIn(ctx *context.Context) {
	// Если вкелючен OneWork, то не нужно показывать ванильную страницу авторизации
	if setting.OneWork.Enabled {
		ctx.HTML(http.StatusOK, "status/404")
		return
	}
	if setting.IAM.Enabled && !setting.IAM.AuthorizationFormEnabled {
		ctx.HTML(http.StatusOK, "status/404")
		return
	}
...
func SignInPost(ctx *context.Context) {
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Error has occurred while getting active OAuth2 providers",
		}
	...
	if err != nil {
		...
		if user_model.IsErrUserNotExist(err) || user_model.IsErrEmailAddressNotExist(err) {
			...
			if setting.Service.RegisterEmailConfirm {
				...
				auditParams["error"] = "Not active account"
			} else {
				log.Info("Failed authentication attempt for %s from %s: %v", form.UserName, ctx.RemoteAddr(), err)
				ctx.Data["Title"] = ctx.Tr("auth.prohibit_login")
				ctx.HTML(http.StatusOK, "user/auth/prohibit_login")
	...
	if err != nil {
		ctx.ServerError("UserSignIn", err)
		auditParams := map[string]string{
			"error": "User is enrolled in 2FA TOTP",
		}
	...
	if err != nil {
		ctx.ServerError("UserSignIn", err)
		auditParams := map[string]string{
			"error": "User has webauthn registration",
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, u.Name, strconv.FormatInt(u.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
...
func SignOut(ctx *context.Context) {
	if ctx.Doer != nil {
		...
		if ctx.Doer.LoginType == auth.OAuth2 {
			sources, err := auth.GetActiveOAuth2ProviderSources()
			if err == nil && sources != nil {
				for _, source := range sources {
					if source.Type == auth.OAuth2 {
						if token := ctx.Session.Get(RefTokenKey); token != nil {
							_ = source.Cfg.(*oauth2.Source).LogOut(ctx.Req, token.(string))
						}
					}
				}
			}
	...
	audit.CreateAndSendEvent(audit.UserLogoutEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, auditParams)
	ctx.Redirect(setting.AppSubURL + "/")
}
...
func SignUpPost(ctx *context.Context) {
	...
	if form.Email == "" && !setting.SourceControl.EmptyEmailEnabled {
		ctx.RenderWithErr(ctx.Tr("user.empty_email_enabled"), tplSignUp, &form)
		return
	}
	ctx.Data["Title"] = ctx.Tr("sign_up")
...
func createUserInContext(ctx *context.Context, tpl base.TplName, form interface{}, u *user_model.User, overwrites *user_model.CreateUserOverwriteOptions, gothUser *goth.User, allowLink bool) (ok bool) {
	...
	if err := user_model.CreateUser(u, overwrites); err != nil {
		if allowLink && (user_model.IsErrUserAlreadyExist(err) || user_model.IsErrEmailAlreadyUsed(err)) {
			...
			auditParams["error"] = "User is already exist or email is already used"
