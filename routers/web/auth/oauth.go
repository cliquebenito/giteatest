...
func SignInOAuth(ctx *context.Context) {
	...
	if err != nil {
		...
		auditParams := map[string]string{
			"error": "Error has occurred while getting active OAuth2 source by provider name",
		}
	...
	if err = authSource.Cfg.(*oauth2.Source).Callout(ctx.Req, ctx.Resp); err != nil {
		if strings.Contains(err.Error(), "no provider for ") {
			if err = oauth2.ResetOAuth2(); err != nil {
				ctx.ServerError("SignIn", err)
				auditParams := map[string]string{
					"error": "Error has occurred while resetting OAuth2",
				}
				audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
				return
			}
			if err = authSource.Cfg.(*oauth2.Source).Callout(ctx.Req, ctx.Resp); err != nil {
				ctx.ServerError("SignIn", err)
				auditParams := map[string]string{
					"error": "Error has occurred while redirecting request/response pair to authenticate against the provider",
				}
				audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			}
			return
...
func SignInOAuthCallback(ctx *context.Context) {
	...
	if err != nil {
		...
		if user_model.IsErrUserProhibitLogin(err) {
			...
			auditParams["error"] = "Prohibit login"
			audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
		...
		auditParams["error"] = "Error has occurred while handling the callback from the OAuth2 provider"
		audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if u == nil {
		if ctx.Doer != nil {
			...
			if err != nil {
				...
				auditParams["error"] = "Error has occurred while linking account to user"
				audit.CreateAndSendEvent(audit.UserLoginEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
				return
			}

			ctx.Redirect(setting.AppSubURL + "/user/settings/security")
			audit.CreateAndSendEvent(audit.UserLoginEvent, ctx.Doer.Name, strconv.FormatInt(ctx.Doer.ID, 10), audit.StatusSuccess, ctx.Req.RemoteAddr, auditParams)
			return
			...
			if len(missingFields) > 0 {
				...
				auditParams := map[string]string{
					"error": "OAuth2 Provider returned empty or missing fields",
				}
				audit.CreateAndSendEvent(audit.UserLoginEvent, audit.EmptyRequiredField, audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
				return
			}
...
func showLinkingLogin(ctx *context.Context, gothUser goth.User) {
	if err := updateSession(ctx, nil, map[string]interface{}{
		...
		auditParams := map[string]string{
			"error": "Error has occurred while updating session",
			"email": gothUser.Email,
		}
		audit.CreateAndSendEvent(audit.UserLoginEvent, getUserName(&gothUser), audit.EmptyRequiredField, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
...
func handleOAuth2SignIn(ctx *context.Context, source *auth.Source, u *user_model.User, gothUser goth.User) {
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while unmarshalling group team mapping"
		audit.CreateAndSendEvent(audit.UserLoginEvent, u.Name, strconv.FormatInt(u.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if !needs2FA {
		if err := updateSession(ctx, nil, map[string]interface{}{
			"uid":       u.ID,
			"uname":     u.Name,
			RefTokenKey: gothUser.RefreshToken,
		}); err != nil {
			ctx.ServerError("updateSession", err)
		...
		if err := resetLocale(ctx, u); err != nil {
			...
			auditParams["error"] = "Error has occurred while resetting locale"
			audit.CreateAndSendEvent(audit.UserLoginEvent, u.Name, strconv.FormatInt(u.ID, 10), audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
