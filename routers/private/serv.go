...
func NewServer(creator taskCreator) Server {
	return Server{taskCreator: creator}
}
...
func (s Server) ServCommand(ctx *context.PrivateContext) {
	...
	if setting.SourceControl.TenantWithRoleModeEnabled {
		if user != nil && owner.IsOrganization() {
			...
			allowed, err := role_model.CheckUserPermissionToOrganization(ctx, user, tenantId, &organization.Organization{ID: owner.ID}, action)
			if err != nil {
				ctx.JSON(http.StatusForbidden, private.Response{
					UserMsg: "The repository you are trying to reach either does not exist or you are not authorized to view it.",
				})
			...
			if !allowed {
				allow, err := role_model.CheckUserPermissionToTeam(ctx, user, tenantId, &organization.Organization{ID: owner.ID}, &repo_model.Repository{ID: repo.ID}, role_model.ViewBranch.String())
				if err != nil || !allow {
	...
	if user != nil {
		if err = s.taskCreator.Create(ctx, repo.ID, user.ID, code_hub_counter_task.CloneRepositoryAction); err != nil {
