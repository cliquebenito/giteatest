...
func checkDatabase(ctx *context.Context, form *forms.InstallForm) bool {
	...
	if hasPostInstallationUser && dbMigrationVersion > 0 {
		log.Error("The database is likely to have been used by SourceControl before, database migration version=%d", dbMigrationVersion)
		...
		log.Info("User confirmed reinstallation of SourceControl into a pre-existing database")
	}

	if hasPostInstallationUser || dbMigrationVersion > 0 {
		log.Info("SourceControl will be installed in a database with: hasPostInstallationUser=%v, dbMigrationVersion=%v", hasPostInstallationUser, dbMigrationVersion)
	}
