...
func CreateUserRepo(ctx *context.APIContext, owner *user_model.User, opt api.CreateRepoOption) {
	...
	if err != nil {
		...
		} else if repo_model.IsErrCreateUserRepo(err) {
...
func Create(ctx *context.APIContext) {
	...
	if err != nil {
		log.Error("Error has occurred while getting user's tenant id: %v", err)
		ctx.Error(http.StatusInternalServerError, "Error has occurred while getting user's tenant id", err)
		return
	}

	allowed, err := role_model.CheckUserPermissionToOrganization(ctxTrace, ctx.Doer, tenantID, &organization.Organization{ID: ctx.ContextUser.ID}, role_model.CREATE)
	if err != nil {
		log.Error("Error has occurred while checking user's permissions: %v", err)
		ctx.Error(http.StatusInternalServerError, "Error has occurred while checking user's permissions", err)
		return
	}

	if !allowed {
		log.Error("Given user is not allowed to access this organization to create repository")
		ctx.Error(http.StatusForbidden, "", "Given user is not allowed to access this organization to create repository")
		return
	}
...
func CreateOrgRepo(ctx *context.APIContext) {
	...
	if !organization.HasOrgOrUserVisible(ctx, org.AsUser(), ctx.Doer) {
		log.Error("User doesn't have permissions to watch a private project which hasn't been created by him")
		ctx.Error(http.StatusForbidden, "", "User doesn't have permissions to watch a private project")
		return
	}
	...
	tenantID, err := tenant_model.GetTenantByOrgIdOrDefault(db.DefaultContext, org.ID)
	if err != nil {
		log.Error("Error has occurred while getting tenant by ord_id %d: %v", org.ID, err)
		ctx.Error(http.StatusInternalServerError, "Error has occurred while getting tenant by ord_id", err)
		return
	}

	allowed, err := role_model.CheckUserPermissionToOrganization(ctx, &user_model.User{ID: ctx.Doer.ID}, tenantID, org, role_model.CREATE)
	if err != nil {
		log.Error("Error has occurred while checking user permissions to create repository: %v", err)
		ctx.Error(http.StatusInternalServerError, "Error has occurred while checking user's permissions", err)
		return
	}
...
func updateBasicProperties(ctx *context.APIContext, opts api.EditRepoOption) error {
	...
	if ctx.Repo.GitRepo == nil && !repo.IsEmpty {
		var err error
		ctx.Repo.GitRepo, err = git.OpenRepository(ctx, ctx.Repo.Repository.OwnerName, ctx.Repo.Repository.Name, ctx.Repo.Repository.RepoPath())
		if err != nil {
