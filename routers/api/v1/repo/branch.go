...
func GetBranch(ctx *context.APIContext) {
	...
	branchProtection, err := git_model.GetMergeMatchProtectedBranchRule(ctx, ctx.Repo.Repository.ID, branchName)
	if err != nil {
		log.Error("Err: get merge math protected branch with repo id - %d, branch name - %s: %v", ctx.Repo.Repository.ID, branchName, err)
...
func DeleteBranch(ctx *context.APIContext) {
	...
	if err := repo_service.DeleteBranch(ctx, ctx.Doer, ctx.Repo.Repository, ctx.Repo.GitRepo, branchName); err != nil {
		...
		case protected_branch.IsBranchIsProtectedError(err):
			log.Warn("Protected branch with name - %s, is protected", branchName)
...
func CreateBranch(ctx *context.APIContext) {
	...
	if err = repo_service.CreateNewBranchFromCommit(ctx, ctx.Doer, ctx.Repo.Repository, oldCommit.ID.String(), opt.BranchName); err != nil {
	...
	branchProtection, err := git_model.GetMergeMatchProtectedBranchRule(ctx, ctx.Repo.Repository.ID, branch.Name)
	if err != nil {
		log.Error("Err: get merge math protected branch with repo id - %d, branch name - %s: %v", ctx.Repo.Repository.ID, branch.Name, err)
		ctx.Error(http.StatusInternalServerError, "GetBranchProtection", err)
		return
	}
...
func ListBranches(ctx *context.APIContext) {
	...
	if !ctx.Repo.Repository.IsEmpty && ctx.Repo.GitRepo != nil {
		...
		for i := range branches {
			...
			branchProtection := git_model.GetFirstMatched(rules, branches[i].Name)
			apiBranch, err := convert.ToBranch(ctx, ctx.Repo.Repository, branches[i], c, branchProtection, ctx.Doer, ctx.Repo.IsAdmin())
			if err != nil {
				log.Error("Err: get first matched with branch name - %s: %v", branches[i].Name, err)
				ctx.Error(http.StatusInternalServerError, "convert.ToBranch", err)
				return
			}
...
func CreateBranchProtection(ctx *context.APIContext) {
	...
	if isPlainRule {
		isBranchExist = git.IsBranchExist(ctx.Req.Context(), ctx.Repo.Repository.OwnerName, ctx.Repo.Repository.Name, ctx.Repo.Repository.RepoPath(), ruleName)
	}
	...
	if err = git_model.UpdateProtectBranch(ctx, ctx.Repo.Repository, protectBranch, protected_branch.WhitelistOptions{
		UserIDs:          whitelistUsers,
		MergeUserIDs:     mergeWhitelistUsers,
		ApprovalsUserIDs: approvalsWhitelistUsers,
	}); err != nil {
		log.Error("Err: update protect branch: %v", err)
...
func EditBranchProtection(ctx *context.APIContext) {
	...
	var isBranchExist bool
	if isPlainRule {
		isBranchExist = git.IsBranchExist(ctx.Req.Context(), ctx.Repo.Repository.OwnerName, ctx.Repo.Repository.Name, ctx.Repo.Repository.RepoPath(), bpName)
	}
