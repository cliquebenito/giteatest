...
func (c *Commit) Parent(n int) (*Commit, error) {
	...
	parent, err := c.repo.getCommit(id.String())
	if err != nil {
		return nil, err
	}
	return parent, nil
...
func CommitsCount(ctx context.Context, opts CommitsCountOptions) (int64, error) {
	...
	ctxWithCancel, cancel := context.WithCancel(opts.Ctx)
	defer cancel()

	//todo may be refactor for
	if len(opts.Revision) > 0 && len(opts.RelPath) > 0 {
		for _, rev := range opts.Revision {
			for _, path := range opts.RelPath {
				commitsResponse, err := opts.CommitClient.CountCommits(ctxWithCancel, &gitalypb.CountCommitsRequest{
					Repository: opts.GitalyRepo,
					Revision:   []byte(rev),
					Path:       []byte(path),
				})
				if err != nil {
				...
				count += int64(commitsResponse.Count)
			}
		}
	} else if len(opts.Revision) > 0 {
		for _, rev := range opts.Revision {
			commitsResponse, err := opts.CommitClient.CountCommits(ctxWithCancel, &gitalypb.CountCommitsRequest{
				Repository: opts.GitalyRepo,
				Revision:   []byte(rev),
...
func (c *Commit) CommitsCount() (int64, error) {
	return CommitsCount(c.repo.Ctx, CommitsCountOptions{
		...
		GitalyRepo:   c.repo.GitalyRepo,
		Ctx:          c.repo.Ctx,
		CommitClient: c.repo.CommitClient,
	})
}
...
func (c *Commit) HasPreviousCommit(commitHash SHA1) (bool, error) {
	...
	ctx, cancel := context.WithCancel(c.repo.Ctx)
	defer cancel()
	commitIsAncestor, err := c.repo.CommitClient.CommitIsAncestor(ctx, &gitalypb.CommitIsAncestorRequest{
		Repository: c.repo.GitalyRepo,
		AncestorId: that,
		ChildId:    this,
	})
	if err != nil {
		log.Error("Error has occurred while checking if %s is an ancestor of %s", this, that)
		return false, fmt.Errorf("failed to check commit is ancestor: %w", err)
	}
	return commitIsAncestor.GetValue(), nil
}
...
func (c *Commit) GetBranchName() (string, error) {
	ctx, cancel := context.WithCancel(c.repo.Ctx)
	defer cancel()
	containingCommitClient, err := c.repo.RefClient.ListBranchNamesContainingCommit(ctx, &gitalypb.ListBranchNamesContainingCommitRequest{
		Repository: c.repo.GitalyRepo,
	...
	containingCommitResponse, err := containingCommitClient.Recv()
	if err != nil {
		return "", err
	}

	return string(containingCommitResponse.GetBranchNames()[0]), nil
}
...
func (c *Commit) GetTagName() (string, error) {
	ctx, cancel := context.WithCancel(c.repo.Ctx)
	defer cancel()
	containingCommitClient, err := c.repo.RefClient.ListTagNamesContainingCommit(ctx, &gitalypb.ListTagNamesContainingCommitRequest{
		Repository: c.repo.GitalyRepo,
		CommitId:   c.ID.String(),
	})
	if err != nil {
	...
	return strings.TrimSpace(string(containingCommitResponse.GetTagNames()[0])), nil
}
...
func GetCommitFileStatus(ctx context.Context, repoPath, commitID string) (*CommitFileStatus, error) {
	splitedPath := strings.SplitN(repoPath, "/", 2)
	if len(splitedPath) != 2 {
		log.Error("Error has occurred while parsing repository path: %v", repoPath)
		return nil, fmt.Errorf("invalid repository path: %v", repoPath)
	}

	username := strings.ToLower(splitedPath[0])
	reponame := strings.ToLower(strings.TrimSuffix(splitedPath[1], ".git"))

	gitRepo, err := OpenRepository(ctx, username, reponame, repoPath)
	if err != nil {
	...
	req := make([]*gitalypb.FindChangedPathsRequest_Request, 0, 1)
	req = append(req,
	...
	paths, err := gitRepo.DiffClient.FindChangedPaths(gitRepo.Ctx, &gitalypb.FindChangedPathsRequest{
		Repository: gitRepo.GitalyRepo,
		Commits:    nil,
		Requests:   req,
	})
	if err != nil {
		log.Error("Error has occurred while finding changed paths: %v", err)
		return nil, fmt.Errorf("failed to find changed paths: %w", err)
	}

	recv, err := paths.Recv()
	if err != nil {
		log.Error("Error has occurred while receiving founded changed paths: %v", err)
		return nil, fmt.Errorf("failed to receive founded changed paths: %w", err)
	}

	for _, path := range recv.GetPaths() {
		switch path.Status {
		case gitalypb.ChangedPaths_ADDED:
			fileStatus.Added = append(fileStatus.Added, string(path.Path))
		case gitalypb.ChangedPaths_MODIFIED:
			fileStatus.Modified = append(fileStatus.Modified, string(path.Path))
		case gitalypb.ChangedPaths_DELETED:
			fileStatus.Removed = append(fileStatus.Removed, string(path.Path))
		default: // для кейсов gitalypb.ChangedPaths_TYPE_CHANGE, gitalypb.ChangedPaths_COPIED
			fileStatus.Modified = append(fileStatus.Modified, string(path.Path))
		}
	}
...
func GetFullCommitID(ctx context.Context, repoPath, shortID string) (string, error) {
	path := strings.Split(strings.TrimPrefix(repoPath, setting.RepoRootPath), "/")
	if len(path) != 3 {
		return "", fmt.Errorf("incorrect repo path: %s", repoPath)
	}

	gitRepo, err := OpenRepository(ctx, path[1], strings.Trim(path[2], ".git"), repoPath)
	if err != nil {
		return "", err
	}
	...
	defer cancel()
	commitResponse, err := gitRepo.CommitClient.FindCommit(ctxWithCancel, &gitalypb.FindCommitRequest{Repository: gitRepo.GitalyRepo, Revision: []byte(shortID), Trailers: true})
	if err != nil {
		return "", err
	}
	...
	return strings.TrimSpace(commitResponse.Commit.Id), nil
