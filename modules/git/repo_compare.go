...
func (repo *Repository) GetMergeBase(tmpRemote, base, head string) (string, string, error) {
	...
	revisions = append(revisions, []byte(base))
	revisions = append(revisions, []byte(head))

	mergeBase, err2 := repo.RepoClient.FindMergeBase(repo.Ctx, &gitalypb.FindMergeBaseRequest{
	...
	return strings.TrimSpace(mergeBase.GetBase()), base, err2
}
...
func (repo *Repository) GetCompareInfo(basePath, baseBranch, headBranch string, directComparison, fileOnly bool) (_ *CompareInfo, err error) {
	...
	if err == nil {
		...
		if !fileOnly {
			...
			revisions = append(revisions, "^"+baseCommitID)
			revisions = append(revisions, headBranch)

			ctx, cancel := context.WithCancel(repo.Ctx)
			defer cancel()
			listCommits, err := repo.CommitClient.ListCommits(ctx, &gitalypb.ListCommitsRequest{
				Repository: repo.GitalyRepo,
				Revisions:  revisions,
			})
			if err != nil {
			...
			if err == io.EOF || recv == nil {
				...
				compareInfo.Commits, err = repo.ParseGitCommitsToCommit(recv.GetCommits())
				if err != nil {
					return nil, err
				}
			}
	...
	diffStats, err := repo.DiffClient.DiffStats(repo.Ctx, &gitalypb.DiffStatsRequest{
		Repository:    repo.GitalyRepo,
		LeftCommitId:  compareInfo.MergeBase,
...
func (repo *Repository) GetFilesChangedBetween(base, head string) ([]string, error) {
	diffStatStream, err := repo.DiffClient.DiffStats(repo.Ctx, &gitalypb.DiffStatsRequest{
		Repository:    repo.GitalyRepo,
		LeftCommitId:  base,
		RightCommitId: head,
	})
	if err != nil {
		log.Error("Error has occurred while requesting diff stats: %v", err)
		return nil, fmt.Errorf("failed to request diff stats: %w", err)
	}

	stats := make([]*gitalypb.DiffStats, 0)
	canRead := true
	for canRead {
		diffStatsResponse, err := diffStatStream.Recv()
		if err != nil && err != io.EOF {
			log.Error("Error has occurred while getting diff stats: %v", err)
			return nil, fmt.Errorf("failed to get diff stats: %w", err)
		}
		if diffStatsResponse == nil {
			canRead = false
		} else {
			stats = append(stats, diffStatsResponse.GetStats()...)
		}
	}
