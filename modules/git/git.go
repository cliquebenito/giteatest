...
// Features структура для проверки изменений объектов
...
func SetExecutablePath(path string) error {
	...
	if gitVersion.LessThan(versionRequired) {
		...
		return fmt.Errorf("installed git version %q is not supported, SourceControl requires git version >= %q, %s", gitVersion.Original(), RequiredVersion, moreHint)
	}
...
func syncGitConfig() (err error) {
	...
	for configKey, defaultValue := range map[string]string{
		"user.name":  "SourceControl",
		"user.email": "sourcecontrol@fake.local",
	} {
...
// DefaultFeatures returns the default features
func DefaultFeatures() *Features {
	if defaultFeatures != nil {
		return defaultFeatures
	}
	if !setting.IsProd || setting.IsInTesting {
		log.Warn("default feature is initializing with default values")
	}
	if err := InitSimple(context.Background()); err != nil {
		log.Fatal("Failed while initializing git module by simple steps: %v", err)
	}
	return defaultFeatures
}
