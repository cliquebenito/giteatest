...
func GetSbtMaxSize(field reflect.StructField) string {
	return getRuleBody(field, "SbtMaxSize(")
}
...
func Validate(errs binding.Errors, data map[string]interface{}, f Form, l translation.Locale) binding.Errors {
	...
	if field, ok := typ.FieldByName(errs[0].FieldNames[0]); ok {
		...
		if fieldName != "-" {
			...
			case validation.ErrSbtGitRefName:
				data["ErrorMsg"] = trName + l.Tr("form.git_ref_name_error")
			...
			case validation.ErrSbtMaxSize:
				data["ErrorMsg"] = l.Tr("form.BranchName") + l.Tr("form.max_length_error", GetSbtMaxSize(field))
			case validation.ErrSbtMinSize:
				data["ErrorMsg"] = errs[0].Message
			case validation.ErrSbtRange:
				data["ErrorMsg"] = errs[0].Message
			case validation.ErrSbtUrl:
				data["ErrorMsg"] = errs[0].Message
