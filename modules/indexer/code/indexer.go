		...
		case "bleve":
			...
			defer func() {
				if err := recover(); err != nil {
					...
					log.Error("You can completely remove the \"%s\" directory to make SourceControl recreate the indexes", setting.Indexer.RepoPath)
				}
			}()
		...
		case "elasticsearch":
			...
			defer func() {
				if err := recover(); err != nil {
					log.Error("PANIC whilst initializing repository indexer: %v\nStacktrace: %s", err, log.Stack(2))
					log.Error("The indexer files are likely corrupted and may need to be deleted")
					log.Error("You can completely remove the \"%s\" index to make SourceControl recreate the indexes", setting.Indexer.RepoConnStr)
				}
			}()
