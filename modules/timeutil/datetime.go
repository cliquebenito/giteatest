...
func DateTime(format string, datetime any) template.HTML {
	...
	case "narrow":
		return template.HTML(fmt.Sprintf(`<relative-time format="datetime" weekday="" year="2-digit" month="2-digit" day="2-digit" hour="2-digit" minute="2-digit" second="" datetime="%s">%s</relative-time>`, datetimeEscaped, textEscaped))
	}
