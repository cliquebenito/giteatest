...
func (c *RedisCacher) Delete(key string) error {
	...
	if strings.HasSuffix(key, "*") {
		iterator := c.c.Scan(graceful.GetManager().HammerContext(), 0, key, 0).Iterator()
		for iterator.Next(graceful.GetManager().HammerContext()) {
			c.c.Del(graceful.GetManager().HammerContext(), iterator.Val())
		}
		return iterator.Err()
	}
