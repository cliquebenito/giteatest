...
func HandleOrgAssignment(ctx *Context, args ...bool) {
	...
	if ctx.Org.IsMember {
		...
		// если у нас включена ролевая модель SourceControl, то команда "Owners" считается технической и не отображается
		if setting.SourceControl.TenantWithRoleModeEnabled {
			filteredTeams := make([]*organization.Team, 0, len(ctx.Org.Teams))
			for _, team := range ctx.Org.Teams {
				if team.Name != organization.OwnerTeamName {
					filteredTeams = append(filteredTeams, team)
				}
			}
			ctx.Org.Teams = filteredTeams
			ctx.Org.Organization.NumTeams = len(filteredTeams)
		}
	}
