...
type Tenant struct {
	Tenant tenant.ScTenantOrganizations
}

type RolePermissioner interface {
...
func (r *Repository) CanCommitToBranch(ctx context.Context, doer *user_model.User) (CanCommitToBranchResults, error) {
	protectedBranch, err := git_model.GetMergeMatchProtectedBranchRule(ctx, r.Repository.ID, r.BranchName)
	if err != nil {
	...
	if protectedBranch != nil {
		...
		userCanPush = git_model.CanUserPush(ctx, *protectedBranch, doer)
		requireSigned = protectedBranch.RequireSignedCommits
	}

	sign, keyID, _, err := asymkey_service.SignCRUDAction(ctx, r.Repository.OwnerName, r.Repository.Name, r.Repository.RepoPath(), doer, r.Repository.RepoPath(), git.BranchPrefix+r.BranchName)
...
func (r *Repository) GetCommitGraphsCount(ctx context.Context, hidePRRefs bool, branches, files []string) (int64, error) {
	...
	return cache.GetInt64(cacheKey, func() (int64, error) {
		if len(branches) == 0 {
			return r.GitRepo.GetAllCommitsCount()
		}
		return git.CommitsCount(ctx,
			git.CommitsCountOptions{
				...
				Revision:     branches,
				RelPath:      files,
				GitalyRepo:   r.GitRepo.GitalyRepo,
				Ctx:          r.GitRepo.Ctx,
				CommitClient: r.GitRepo.CommitClient,
			})
	})
}
...
// CanUseBranchWriteActionButton функция проверяет при включенной ролевой модели можно ли пользователю показывать кнопки write-действий с ветками
func (r *Repository) CanUseBranchWriteActionButton(ctx context.Context, roleManager RolePermissioner, user *user_model.User) (bool, error) {
	if setting.SourceControl.Enabled && setting.SourceControl.TenantWithRoleModeEnabled {
		...
		return r.checkCanUseRepoWriteActions(ctx, roleManager, user)
	}
	return r.CanWrite(unit_model.TypeCode), nil
}

// CanUseBranchPullActonButton функция проверяет при включенной ролевой модели можно ли пользователю показывать кнопки pull-действий с ветками
func (r *Repository) CanUseBranchPullActonButton(ctx context.Context, roleManager RolePermissioner, user *user_model.User) (bool, error) {
	if setting.SourceControl.Enabled && setting.SourceControl.TenantWithRoleModeEnabled {
		logTracer := trace.NewLogTracer()
		message := logTracer.CreateTraceMessage(ctx)
		err := logTracer.Trace(message)
	...
	return r.CanWrite(unit_model.TypeCode) || repo_model.HasForkedRepo(user.ID, r.Repository.ID), nil
}
...
func (r *Repository) checkCanUseRepoWriteActions(ctx context.Context, permissioner RolePermissioner, user *user_model.User) (bool, error) {
	logTracer := trace.NewLogTracer()
	message := logTracer.CreateTraceMessage(ctx)
	err := logTracer.Trace(message)
	if err != nil {
	...
	if r == nil || r.Owner == nil {
		return false, fmt.Errorf("get owner of repo")
	}
	orgID := r.Owner.ID
	org, err := organization.GetOrgByID(ctx, orgID)
	if err != nil {
		log.Error("Error has occurred while getting organization by id: %d", orgID)
		return false, fmt.Errorf("get organization by: id %w", err)
	}
	tenantID, err := tenant.GetTenantByOrgIdOrDefault(ctx, orgID)
	if err != nil {
		log.Error("Error has occurred while getting tenant by organization id: %d", orgID)
		return false, fmt.Errorf("get tenant by organization id: %w", err)
	}

	perm, err := permissioner.CheckUserPermissionToOrganization(ctx, user, tenantID, org, role_model.WRITE)
	if err != nil {
		log.Error("Error has occurred while checking user permission to branch write buttons")
		return false, fmt.Errorf("check user permission to branch write buttons: %w", err)
	}
	return perm, nil
}
...
func repoAssignment(ctx *Context, repo *repo_model.Repository) {
	...
	var doer, doerID string
	if ctx.Doer != nil {
		doer = ctx.Doer.Name
		doerID = strconv.FormatInt(ctx.Doer.ID, 10)
	} else {
		doer = audit.EmptyRequiredField
		doerID = audit.EmptyRequiredField
	}

	isHome := ctx.Link == repo.Link()

	if err = repo.LoadOwner(ctx); err != nil {
		ctx.ServerError("LoadOwner", err)
		if isHome {
			auditParams["error"] = "Error has occurred while loading owner"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		}
		return
	}

	// если у нас включена ролевая модель SourceControl, то запускается проверка привилегий на чтение репозитория
	if setting.SourceControl.TenantWithRoleModeEnabled {
		if ctx.IsSigned {
			...
			tenantIDs, err := role_model.GetUserTenantIDsOrDefaultTenantID(ctx.Doer)
			if err != nil {
				log.Error("Error has occurred while getting tenant ids by user: %v", err)
				ctx.Error(http.StatusNotFound, fmt.Sprintf("Error has occurred while getting tenant id by user: %v", err))
				return
			}
			var countMatch int
			for _, tenantId := range tenantIDs {
				allowed, err := role_model.CheckUserPermissionToOrganization(ctx, ctx.Doer, tenantId, &organization.Organization{ID: repo.OwnerID}, action)
				if err != nil {
				...
				if !allowed {
					allowed, err = role_model.CheckUserPermissionToTeam(ctx, ctx.Doer, tenantId, &organization.Organization{ID: repo.OwnerID}, repo, role_model.ViewBranch.String())
					if err != nil {
						log.Error("Error has occurred while checking user's permissions: %v", err)
						ctx.NotFound(ctx.Req.URL.RequestURI(), nil)
						return
			...
			if countMatch == 0 {
				log.Error("Error has occurred while checking user's permissions")
				ctx.NotFound(ctx.Req.URL.RequestURI(), nil)
				return
			}
		}
	...
	if setting.SourceControl.MultiTenantEnabled {
		tenantIDs, err := role_model.GetUserTenantIDsOrDefaultTenantID(ctx.Doer)
		if err != nil {
			log.Error("Error has occurred while getting tenantID: %v", err)
			ctx.NotFound(ctx.Req.URL.RequestURI(), nil)
			return
		}

		tenantID, err := role_model.GetRepoTenantId(repo)
		if err != nil {
			log.Error("Error has occurred while getting tenantID: %v", err)
			ctx.NotFound(ctx.Req.URL.RequestURI(), nil)
			return
	...
	if err != nil {
		...
		ctx.ServerError("GetUserRepoPermission", err)
		if isHome {
			auditParams["error"] = "Error has occurred while getting user permission"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		}
		return
	}
	...
	if !ctx.Repo.Permission.HasAccess() {
		if ctx.FormString("go-get") == "1" {
			...
			if isHome {
				auditParams["error"] = "User does not have actual access to the requested repository or the owner or repository does not exist at all"
				audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			}
			log.Error("Error has occurred while checking user's access")
			return
		}
		ctx.NotFound("no access right", nil)
		if isHome {
			auditParams["error"] = "No access right"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		}
		log.Error("Error has occurred while checking user's access")
		return
	...
	if repo.IsMirror {
		...
		if err == nil {
			...
			log.Error("Error has occurred while getting mirror by repo: %v", err)
			ctx.ServerError("GetMirrorByRepoID", err)
			if isHome {
				auditParams["error"] = "Error has occurred while getting mirror by repository id"
				audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			}
			return
		}
	...
	if err != nil {
		...
		if isHome {
			auditParams["error"] = "Error has occurred while getting push mirrors by repository id"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		}
		return
	}

	tenantId, err := tenant.GetTenantByOrgIdOrDefault(ctx, repo.Owner.ID)
	if err != nil {
		log.Error("Error has occurred while getting tenant: %v", err)
		ctx.NotFound(ctx.Req.URL.RequestURI(), nil)
		return
	}
...
func RepoAssignment(ctx *Context) (cancel context.CancelFunc) {
	...
	if _, repoAssignmentOnce := ctx.Data["repoAssignmentExecuted"]; repoAssignmentOnce {
		...
		auditParams["error"] = "Repo assignment was executed already"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	auditParams["repository"] = repoName
	...
	if ctx.IsSigned && ctx.Doer.LowerName == strings.ToLower(userName) {
		...
		if err != nil {
			...
			auditParams["error"] = "Error has occurred while getting owner by name"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	}
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while getting repository by name"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	if setting.SourceControl.MultiTenantEnabled && repo != nil {
		action := role_model.READ
		if repo.IsPrivate {
			action = role_model.READ_PRIVATE
		}
		...
		for i, id := range tenantIDs {
			...
			if role_model.ConvertAndCheckPrivileges(ctx, ctx.Doer.ID, tenantID, repo.OwnerID, action.String()) {
				break
			}
			if i == len(tenantIDs)-1 {
				ctx.NotFound(ctx.Req.URL.RequestURI(), nil)
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while getting release count by repository id"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if repo.Owner.IsOrganization() {
		...
		if err != nil {
			ctx.ServerError("has occurred while getting tenant", err)
			auditParams["error"] = "Error has occurred while checking permission user fork repository"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
		ctx.Data["TenantID"] = tenantId
	...
	if err != nil {
		ctx.ServerError("has occurred while checking ability user fork repository", err)
		auditParams["error"] = "Error has occurred while checking ability user fork repository"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}

	if canSignedUserFork && (setting.SourceControl.Enabled || setting.SourceControl.TenantWithRoleModeEnabled || setting.SourceControl.MultiTenantEnabled) {
		message := logTracer.CreateTraceMessage(ctx)
		errTrace := logTracer.Trace(message)
		if errTrace != nil {
			log.Error("Error has occurred while creating trace message: %v", errTrace)
		...
		canSignedUserFork, err = repo_module.CheckUserForkRepository(ctx, ctx.Doer, ctx.Repo.Owner.ID, tenantId)
		if err != nil {
			ctx.ServerError("has occurred while checking permission user fork repository in source control", err)
			auditParams["error"] = "Error has occurred while checking ability user fork repository"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	...
	if err != nil {
		ctx.ServerError("has occurred while getting forks by user and organizations", err)
		auditParams["error"] = "Error has occurred while getting forks by user and organizations"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if repo.IsFork {
		...
		if ctx.Written() {
			auditParams["error"] = "Error has occurred while retrieving base repository"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	}
	...
	if repo.IsGenerated() {
		...
		if ctx.Written() {
			auditParams["error"] = "Error has occurred while retrieving template repository"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
	}
	...
	gitRepo, err := git.OpenRepository(ctx, userName, repoName, repo_model.RepoPath(userName, repoName))
	if err != nil {
		...
		ctx.ServerError("has occurred while opening repository "+repo_model.RepoPath(userName, repoName), err)
		auditParams["error"] = "Error has occurred while opening repository"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while getting tag names by repository id"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	ctx.Data["Tags"] = tags
	...
	if err != nil {
		...
		auditParams["error"] = "Error has occurred while getting branches"
		audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
		return
	}
	...
	if ctx.Repo.Repository.Status == repo_model.RepositoryPendingTransfer {
		...
		if err != nil {
			...
			auditParams["error"] = "Error has occurred while getting pending repository transfer"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
		...
		if err := repoTransfer.LoadAttributes(ctx); err != nil {
			...
			auditParams["error"] = "Error has occurred while loading attributes"
			audit.CreateAndSendEvent(audit.RepositoryOpenEvent, doer, doerID, audit.StatusFailure, ctx.Req.RemoteAddr, auditParams)
			return
		}
