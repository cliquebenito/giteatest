...
var (
	...
	AppPath string
	// AppWorkPath is the "working directory" of Gitea. It maps to the environment variable SC_WORK_DIR.
...
func getAppPath() (string, error) {
	...
	//	which path starts with two "/" in Windows: "//psf/HomeOrg/..."
...
func getWorkPath(appPath string) string {
	...
	if giteaWorkPath, ok := os.LookupEnv("SC_WORK_DIR"); ok {
...
// SC_CUSTOM environment variable and with provided overrides before stepping
// back to the default
func SetCustomPathAndConf(providedCustom, providedConf, providedWorkPath string) {
	if len(providedWorkPath) != 0 {
		AppWorkPath = filepath.ToSlash(providedWorkPath)
	}
	if giteaCustom, ok := os.LookupEnv("SC_CUSTOM"); ok {
...
func Init(opts *Options) {
	...
	if !opts.DisableLoadCommonSettings {
		loadCommonSettingsFrom(CfgProvider, opts.ConsoleConf)
	}
}

// loadCommonSettingsFrom loads common configurations from a configuration provider.
func loadCommonSettingsFrom(cfg ConfigProvider, consoleConf bool) {
	...
	loadSourceControl(cfg)
	loadKafka(cfg)
	loadSourceControlWrapVault(cfg)
	loadGitaly(cfg)
	loadIAM(cfg)                  // must be above Task Tracker initialization
	loadSourceControlOneWork(cfg) // must be above Task Tracker initialization
	loadAuditSbtGlobalFrom(cfg)
	loadSourceControlCustomGroups(cfg)
	if !consoleConf {
		loadSourceControlVaultDB(cfg)
		loadSourceControlVaultServer(cfg)
		loadSourceControlVaultSecurity(cfg)
		loadSourceControlVaultOauth2(cfg)
		loadSourceControlAuthKeycloak(cfg)
		loadSourceControlVaultClient()
		loadSbtKeycloakForm(cfg)
		loadKafkaAuthFromVault(cfg)
		loadSourceControlTaskTracker(cfg)
		loadTaskTracker(cfg)
		loadSourceControlVaultMTLS(cfg)
	}
	loadOAuth2From(cfg)
	loadLFSFrom(cfg)
	...
	loadMarkupFrom(cfg)
	loadOtherFrom(cfg)
	loadProofOfWorkFrom(cfg)
	loadSbtOneWorkForm(cfg)
	loadCron(cfg)
	loadCodeHub(cfg)
}
...
func loadRunModeFrom(rootCfg ConfigProvider) {
	...
	if os.Getuid() == 0 {
		if !unsafeAllowRunAsRoot {
			...
			log.Fatal("SourceControl is not supposed to be run as root. Sorry. If you need to use privileged TCP ports please instead use setcap and the `cap_net_bind_service` permission")
		}
		log.Critical("You are running SourceControl using the root user, and have purposely chosen to skip built-in protections around this. You have been warned against this.")
	}
}
