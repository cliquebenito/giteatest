...
var OAuth2 = struct {
	...
	secManClient:               NewGetterForSecMan(),
}
...
func loadOAuth2From(rootCfg ConfigProvider) {
	...
	if CheckSettingsForIntegrationWithSecMan() {
		configForKvGet := &vault_model.KeyValueConfigForGetSecrets{
			SecretPath:  strings.TrimSpace(SourceControlVaultOauth2.SecretPath),
			StoragePath: strings.TrimSpace(SourceControlVaultOauth2.StoragePath),
			VersionKey:  SourceControlVaultOauth2.VersionKey,
		}
		resp, err := OAuth2.secManClient.GetCredFromSecManByVersionKey(configForKvGet)
		if err != nil {
			log.Fatal("Error has occurred while trying to get cred from sec man: %v", err)
		}

		if GetResponseNotNil(resp) {
			OAuth2.JWTSecretBase64 = strings.TrimSpace(resp.Data[SourceControlVaultOauth2.JwtSecret])
			CheckIfSecretIsEmptyAndReportToAudit("JWT_SECRET", OAuth2.JWTSecretBase64, "JWT_SECRET for OAuth2 is empty in secret storage", log.Fatal)
		} else {
			CheckIfSecretIsEmptyAndReportToAudit("JWT_SECRET", "", "Response from secret storage is nil", log.Fatal)
		}
	}
	key := make([]byte, 32)
	...
	if err != nil || n != 32 {
		if !SourceControlWrapVault.Enabled {
			...
			if err != nil {
				log.Fatal("Error has occurred while generating JWT secret: %v", err)
			}

			secretBase64 := base64.RawURLEncoding.EncodeToString(key)
			OAuth2.JWTSecretBase64 = secretBase64
			rootCfg.Section("oauth2").Key("JWT_SECRET").SetValue(secretBase64)
			if err := rootCfg.Save(); err != nil {
				log.Fatal("Error has occurred while saving oauth2.JWT_SECRET: %v", err)
			}
		}
	}
}
