...
func loadSecret(sec ConfigSection, uriKey, verbatimKey string) string {
	...
	if err != nil {
		log.Fatal("Error has occurred while parsing %s (%s): %v", uriKey, uri, err)
	}
	switch tempURI.Scheme {
	case "file":
		buf, err := os.ReadFile(tempURI.RequestURI())
		if err != nil {
			log.Fatal("Error has occurred while reading %s (%s): %v", uriKey, tempURI.RequestURI(), err)
		}
		...
		if val == "" {
			...
			log.Fatal("Error has occurred while reading %s (%s): the file is empty", uriKey, tempURI.RequestURI())
		}
		return val
...
func generateSaveInternalToken(rootCfg ConfigProvider) {
	...
	if err != nil {
		log.Fatal("Error has occurred while generating internal token: %v", err)
	}

	InternalToken = token
	if !SourceControlWrapVault.Enabled {
		rootCfg.Section("security").Key("INTERNAL_TOKEN").SetValue(token)
		if err := rootCfg.Save(); err != nil {
			log.Fatal("Error has occurred while saving internal token: %v", err)
		}
	}
}
	...
	if InstallLock && InternalToken == "" {
		...
		if CheckSettingsForIntegrationWithSecMan() {
			configForKvGet := &vault_model.KeyValueConfigForGetSecrets{
				SecretPath:  strings.TrimSpace(SourceControlVaultSecurity.SecretPath),
				StoragePath: strings.TrimSpace(SourceControlVaultSecurity.StoragePath),
				VersionKey:  SourceControlVaultSecurity.VersionKey,
			}
			secManGet := NewGetterForSecMan()
			resp, err := secManGet.GetCredFromSecManByVersionKey(configForKvGet)
			if err != nil {
				log.Fatal("Error has occurred while tying to get cred from secret storage: %v", err)
			}

			if GetResponseNotNil(resp) {
				InternalToken = strings.TrimSpace(resp.Data[SourceControlVaultSecurity.InternalToken])
				CheckIfSecretIsEmptyAndReportToAudit("INTERNAL_TOKEN", InternalToken, "INTERNAL_TOKEN is empty in secret storage", log.Fatal)
			} else {
				CheckIfSecretIsEmptyAndReportToAudit("INTERNAL_TOKEN", "", "Response from secret storage is nil", log.Fatal)
			}

			errSetInternalToken := os.Setenv("INTERNAL_TOKEN", InternalToken)
			if errSetInternalToken != nil {
				log.Fatal("Error has occurred while adding internal token to env: %v", errSetInternalToken)
			}
		} else {
