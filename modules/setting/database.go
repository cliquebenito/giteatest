...
func loadDBSetting(rootCfg ConfigProvider) {
	...
	if CheckSettingsForIntegrationWithSecMan() {
		configForKvGet := &vault_model.KeyValueConfigForGetSecrets{
			SecretPath:  strings.TrimSpace(SourceControlSecretDB.SecretPath),
			StoragePath: strings.TrimSpace(SourceControlSecretDB.StoragePath),
			VersionKey:  SourceControlSecretDB.VersionKey,
		}
		Database.secManGetter = NewGetterForSecMan()
		resp, err := Database.secManGetter.GetCredFromSecManByVersionKey(configForKvGet)
		if err != nil {
			log.Fatal("loadDBSetting failed when we tried to get cred from sec man: %v", err)
		}

		if GetResponseNotNil(resp) {
			Database.User = resp.Data[SourceControlSecretDB.UserKey]
			CheckIfSecretIsEmptyAndReportToAudit("USER_KEY", Database.User, "USER_KEY to db is empty in secret storage", log.Fatal)

			Database.Passwd = resp.Data[SourceControlSecretDB.PasswordKey]
			CheckIfSecretIsEmptyAndReportToAudit("PASSWORD_KEY", Database.Passwd, "PASSWORD_KEY to db is empty in secret storage", log.Fatal)
		} else {
			CheckIfSecretIsEmptyAndReportToAudit("USER_KEY", "", "Response from secret storage is nil", log.Fatal)
		}
	} else {
	...
	if Database.Type.IsPostgreSQL() {
		Database.Schema = sec.Key("SCHEMA").MustString("public")
	} else {
