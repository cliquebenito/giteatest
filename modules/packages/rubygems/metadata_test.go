...
4ftn7/yUxd4YGcglvTmmY8aGY3ZwRn4CqcWcidUGAAA=`)
	...
	assert.Equal(t, "SourceControl package", rp.Metadata.Summary)
	assert.Equal(t, "RubyGems package test", rp.Metadata.Description)
	assert.Equal(t, []string{"SourceControl"}, rp.Metadata.Authors)
	assert.Equal(t, "https://sourcecontrol.sbertech.ru/", rp.Metadata.ProjectURL)
