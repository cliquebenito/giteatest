...
func generateRepoCommit(ctx context.Context, repo, templateRepo, generateRepo *repo_model.Repository, tmpDir string) error {
	...
	//if err != nil {
	//	return fmt.Errorf("checkSourceControlTemplate: %w", err)
	//}
	//
	//if gt != nil {
	...
	//					}
	//
	//					if err = filesCommit(ctx, repo, []byte(generateExpansion(string(content), []byte(res), path); err != nil {
	//						return fmt.Errorf("create file gitaly gitignore: %w", err)
	//					}
	//					break
	//				}
	//			}
...
func generateGitContent(ctx context.Context, repo, templateRepo, generateRepo *repo_model.Repository) (err error) {
	...
	// если при создании репозитория не была указана ветка по умолчанию, используйте ветку по умолчанию из шаблона
...
func GenerateRepository(ctx context.Context, doer, owner *user_model.User, templateRepo *repo_model.Repository, opts GenerateRepoOptions) (_ *repo_model.Repository, err error) {
	...
	gRepo := MatchRepository(generateRepo.OwnerName, generateRepo.Name)

	err = CheckInitRepository(ctx, gRepo)
	if err != nil {
		return generateRepo, err
	}

	err = CreateRepositoryGitaly(ctx, generateRepo.DefaultBranch, gRepo)
	if err != nil {
		return generateRepo, err
	}
