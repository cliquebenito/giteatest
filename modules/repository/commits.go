...
func (pc *PushCommits) toAPIPayloadCommit(ctx context.Context, repoPath, repoLink string, commit *PushCommit) (*api.PayloadCommit, error) {
	...
	if !ok {
		if commit.AuthorEmail != "" {
			...
			author, err = user_model.GetUserByName(ctx, commit.AuthorName)
			if err == nil {
				authorUsername = author.Name
				pc.emailUsers[commit.AuthorName] = author
			}
		}
	...
	if !ok {
		...
		} else if commit.CommitterName != "" {
			committer, err = user_model.GetUserByName(ctx, commit.CommitterName)
			if err == nil {
				committerUsername = committer.Name
				pc.emailUsers[commit.CommitterName] = committer
			}
		}
	} else {
		committerUsername = committer.Name
...
func (pc *PushCommits) AvatarLink(ctx context.Context, email string) string {
	...
	if !ok {
		...
		if u == nil {
			u, err = user_model.GetUserByName(ctx, pc.HeadCommit.AuthorName)
		}
		if email == "" {
			email = u.Name
		}
