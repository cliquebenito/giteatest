...
import (
	...
	"code.gitea.io/gitea/modules/notification/kafka"
...
func NewContext() {
	...
	RegisterNotifier(kafka.NewNotifier())
}
...
// NotifyPullRequestRebuild notifies when user rebuild pull to notifiers
func NotifyPullRequestRebuild(ctx context.Context, doer *user_model.User, issue *issues_model.Issue, remoteAddress string) {
	for _, notifier := range notifiers {
		notifier.NotifyPullRequestRebuild(ctx, doer, issue, remoteAddress)
	}
}
