	...
	for _, opts := range items {
		if setting.SourceControl.Enabled && setting.SourceControl.TenantWithRoleModeEnabled {
			...
			if opts.ReceiverID != 0 {
				userIDs = append(userIDs, opts.ReceiverID)
			} else {
				userIDs, err = activities_model.GetUsersFromNotificationsByIssueID(ctx, opts.IssueID)
				if err != nil {
					log.Error("Error has occurred while getting watchers of the issue: %v", err)
					return nil
				}
			}

			for idx := range userIDs {
				tenantID, err := role_model.GetUserTenantId(ctx, userIDs[idx])
				if err != nil {
					log.Error("Error has occurred while getting tenant id by user id: %v", err)
					return nil
				}

				repo, err := issues_model.GetRepositoryByIssueID(ctx, opts.IssueID)
				if err != nil {
					log.Error("Error has occurred while getting repository by issue id: %v", err)
					return nil
				}
				...
				//logTrace := models.LogTrace{
				//	TraceId: ctx.Data["TraceID"].(string),
				//	Method:  "allowPermissions",
				//	Params: models.LogParams{
				//		UserId:   ctx.Doer.ID,
				...
				//	},
				//}
				//message, _ := json.Marshal(logTrace)
				//log.Info(string(message))

				ctxTrace := context.WithValue(ctx, trace.Key, opts.TraceID)
				ctxTrace = context.WithValue(context.Background(), trace.EndpointKey, "NotificationService")
				allowed, err := role_model.CheckUserPermissionToOrganization(ctxTrace, &user_model.User{ID: userIDs[idx]},
					tenantID,
				...
				if !allowed {
					allow, err := role_model.CheckUserPermissionToTeam(ctxTrace,
						&user_model.User{ID: userIDs[idx]},
						tenantID,
						&organization.Organization{ID: repo.OwnerID},
				...
				if err := activities_model.CreateOrUpdateIssueNotifications(opts.IssueID, opts.CommentID, opts.NotificationAuthorID, userIDs[idx]); err != nil {
...
// NotifyPullRequestRebuild notify when pull request rebuild manually
func (ns *notificationService) NotifyPullRequestRebuild(ctx context.Context, doer *user_model.User, issue *issues_model.Issue, remoteAddress string) {
	if err := issue.LoadPullRequest(ctx); err != nil {
		log.Error("issue.LoadPullRequest: %v", err)
		return
	}
	if issue.IsPull && !issue.PullRequest.IsWorkInProgress() {
		_ = ns.issueQueue.Push(issueNotificationOpts{
			IssueID:              issue.ID,
			NotificationAuthorID: doer.ID,
		})
